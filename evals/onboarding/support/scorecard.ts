// Scoring for the sampled admission eval (docs/architecture.md §11.3 E4,
...
export function formatScorecard(sc: Scorecard): string {
  const lines = [
    "onboarding eval — admission scorecard",
...
// artifact (admission-eval-nightly.yml, `if: always()`); this module reads no
// environment, so the CI surfaces are the artifact and the job log, never a
