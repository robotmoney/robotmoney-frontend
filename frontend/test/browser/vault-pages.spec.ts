...
const byVaultValue = (page: Page) => byVault(page).locator(".rr-legend__v");
const byVaultTarget = (page: Page) => byVault(page).locator(".rr-legend__t");
...
for (const [i, v] of SLUGS.entries()) {
  test(`${v.slug} on the devnet: its hue, its three layers, labelled once as test data`, async ({ page }) => {
    ...
    await expect(page.locator("#activity thead th:visible")).toHaveText(["Date", "Event", "Shares", "Assets"]);
...
test("devnet-stale and devnet-paused show in the facts", async ({ page }) => {
  ...
  await openVault(page, "rmusdc");
  await expect(fact(page, "As of")).toContainText("delayed");
...
test("devnet-stale and devnet-paused on /allocation: the read says stale, rmUSDC says paused", async ({ page }) => {
  ...
  await vaultsLoaded(page);
  await expect(vaultFact(page, "As of")).toContainText("· delayed");
...
test("the vault subject on the devnet: the router and four vaults, one book grouped by vault", async ({ page }) => {
  ...
  // The one Vaults table: each vault's value, its share of the book, the
  // target in force (the router's applied weights, named Target) and the
  // drift between them.
  ...
  await expect(panel.locator("thead th")).toHaveText(["Constituent", "% of sleeve", "% of total"]);
  ...
  await expect(hold.locator(".rr-positions thead th:visible")).toHaveText(["Constituent", "Amount", "Price", "Value", "Weight"]);
  ...
  await expect(act.locator("thead th:visible")).toHaveText(["Date", "Sleeve", "Event", "Shares", "Assets"]);
...
test("the vault subject on Base: rmUSDC alone, against the weights in force", async ({ page }) => {
  ...
  await expect(hold.locator(".rr-stat__sub").first()).toHaveText(`Base · ${day(SAVED_BASE.asOf)}`);
