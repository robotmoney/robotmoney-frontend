...
//     snapshot (data/vaults/base/vault-economics.json), the archive's
//     2026-06-24 recommendation and the archive's subject book;
//   - "live": the four-vault route is absent (404), vault-economics and the
//     allocation answer with the committed goldens, and the allocation's
//     sessions with one published bucket_weights session.
...
const layerRow = (page: Page, label: string) => page.locator(".rr-layers__row").filter({ hasText: label }).locator(".rr-mono");
const fact = (page: Page, label: string) => page.locator(".rr-head + .rr-meta .rr-meta__i").filter({ hasText: label });
const sideways = (page: Page) =>
  page.evaluate(() => document.documentElement.scrollWidth - document.documentElement.clientWidth);

// /allocation's Vaults section. Its cells in order: TVL, Recommended, Applied,
// Actual, Governance gap, Flow gap.
const vaultRows = (page: Page) => page.locator("#vaults tbody tr");
const vaultCol = (page: Page, n: number) => page.locator(`#vaults tbody tr td:nth-of-type(${n})`);
...
// The Robot Money Vault subject. Its Holdings target follows the page's first
...
async function subjectLoaded(page: Page) {
  ...
  await expect(page.locator("#holdings .rr-legend__row small").first()).toHaveText(/^(target|applied) \d/);
}
...
for (const [i, v] of SLUGS.entries()) {
  test(`${v.slug} on the devnet: its hue, its three layers, labelled once as test data`, async ({ page }) => {
    ...
    await expect(layerRow(page, "Applied")).toHaveText(pct(row.appliedBps));
    await expect(layerRow(page, "Actual")).toHaveText(pct(row.actualBps));
    for (const bar of await page.locator(".rr-layers__bar i").all()) {
      expect(await bar.evaluate((el) => getComputedStyle(el).backgroundColor)).toBe(CATEGORICAL_RGB[i]);
    }

    // The holdings reconcile to the vault's TVL, to the cent.
    await expect(page.locator("#holdings .rr-stat__v")).toHaveText(`$${row.tvlUsd.toLocaleString("en-US", { minimumFractionDigits: 2 })}`);
    const values = await page.locator(".rr-holdings tbody td:nth-child(3)").allTextContents();
    expect(values.length).toBeGreaterThan(0);
    expect(Math.round(values.reduce((s, x) => s + dollars(x), 0) * 100)).toBe(Math.round(row.tvlUsd * 100));
...
test("rmUSDC's devnet gaps and recommendation read as the overview states them", async ({ page }) => {
  ...
  await expect(gaps.locator(".rr-meta__i", { hasText: "Flow gap" })).toContainText("+2 pp");
  await expect(gaps.locator(".alp__mv.up")).toHaveCount(2);
...
async function expectRmusdcOnBase(page: Page, economics: any) {
  ...
  // No router on Base: no Applied layer, and one gap, actual against
  // recommended, in place of the governance and flow gaps.
  await expect(layerRow(page, "Applied")).toHaveCount(0);
  await expect(layerRow(page, "Actual")).toHaveText("100%");
  const gaps = page.locator("#allocation .rr-meta .rr-meta__i");
  await expect(gaps).toHaveCount(1);
  await expect(gaps).toContainText("Gap");
  await expect(gaps).not.toContainText("Governance");
...
test("rmUSDC on Base from the saved snapshot: the adapters, the contract, the deposit skill", async ({ page }) => {
  ...
  // The archive's 2026-06-24 recommendation, 95/3/0/2.
  await expect(layerRow(page, "Recommended")).toHaveText("95%");
  await expect(page.locator("#allocation .rr-meta__i", { hasText: "Gap" })).toContainText("+5 pp");
...
test("rmUSDC on Base from the live feed: the same vault, and no data label", async ({ page }) => {
  ...
  // The stubbed session's weights, linked to the session itself.
  await expect(layerRow(page, "Recommended")).toHaveText("90%");
  await expect(page.locator("#allocation .rr-meta__i", { hasText: "Gap" })).toContainText("+10 pp");
...
test("a stub feed and a stale adapter read say so", async ({ page }) => {
  ...
  await expect(page.locator(".rr-holdings tbody th").first().locator("small")).toHaveText("stale (Jul 30, 4:20 PM UTC)");
...
test("rmAGENT on Base is not live: its allocation and mechanics, nothing to hold", async ({ page }) => {
  ...
  expect(await page.locator("section.rr-sec").evaluateAll((els) => els.map((e) => e.id))).toEqual(["allocation", "mechanics"]);
  await expect(layerRow(page, "Recommended")).toHaveText("3%");
  await expect(layerRow(page, "Actual")).toHaveText("0%");
  await expect(page.locator("#allocation .rr-meta__i", { hasText: "Gap" })).toContainText("−3 pp");
  await expect(page.locator(".rr-disc")).toHaveCount(0);
  await expect(page.locator(".rr-cta")).toHaveCount(0);
  await expectNoBrowserErrors(errors);
});
...
test("/allocation on the devnet: four vaults against the recommendation, labelled as test data", async ({ page }) => {
  ...
  await expect(rows.nth(0).locator("td")).toHaveText(["$72,000.00", "65%", "70%", "72%", "+5 pp", "+2 pp"]);
  await expect(rows.nth(0).locator(".alp__mv.up")).toHaveCount(2);
  // A zero gap is a reading: "0 pp", flat, with no arrow.
  const protoGov = rows.nth(2).locator("td").nth(4).locator(".alp__mv");
  await expect(protoGov).toHaveText("0 pp");
  await expect(protoGov).toHaveClass(/\bflat\b/);
  await expect(protoGov.locator("i")).toHaveCount(0);

  await expect(vaultFact(page, "Combined TVL")).toContainText("$100,000.00");
...
test("/allocation from the saved Base snapshot: rmUSDC alone, the archive's recommendation, no devnet figure", async ({ page }) => {
  ...
  const rows = vaultRows(page);
  await expect(rows.nth(0).locator("td")).toHaveText([usd2(SAVED_BASE.tvlUsd), "95%", "100%", "+5 pp"]);
  // No router on Base: TVL, Recommended, Actual and one Gap.
  ...
  for (const i of [1, 2, 3]) {
    ...
    await expect(rows.nth(i).locator("td").nth(2)).toHaveText("0%");
  }
...
test("/allocation from the live Base feed: no label, the golden's TVL, and no house-book request", async ({ page }) => {
  ...
  await expect(vaultRows(page).nth(0).locator("td").first()).toHaveText(usd2(golden.tvlUsd));
  await expect(vaultFact(page, "Combined TVL")).toContainText(usd2(golden.tvlUsd));
  await expect(vaultCol(page, 2)).toHaveText(["90%", "5%", "3%", "2%"]);
...
test("?vaults=devnet persists for the tab: a later page with no query stays on the devnet", async ({ page }) => {
  ...
  await expect(page.locator("#holdings .rr-stat__v")).toHaveText("$72,000.00");
  await page.locator(".rr-crumbs a").click();
  await expect(page).toHaveURL(/\/allocation#vaults$/);
  await navigate(page, "/vault/rmagent");
  await expect(page.locator("#holdings .rr-stat__v")).toHaveText("$9,000.00");
...
test("devnet-unreadable: rmPROTO cannot be read, so no vault has an actual weight", async ({ page }) => {
  ...
  await navigate(page, "/vault/rmusdc");
  await expect(layerRow(page, "Actual")).toHaveText("—");
  await expect(page.locator("#allocation .rr-meta__i", { hasText: "Flow gap" })).toContainText("—");
});
...
test("devnet-unreadable on /allocation: Actual, the combined TVL and the tracking error are missing", async ({ page }) => {
  ...
  await expect(vaultCol(page, 6)).toHaveText(["—", "—", "—", "—"]);
  ...
  // Recommended does not depend on the vaults' reads.
  await expect(vaultCol(page, 2)).toHaveText(["65%", "15%", "15%", "5%"]);
});
...
test("devnet-no-recommendation: Recommended and the governance gap are missing, not zero", async ({ page }) => {
  ...
  await expect(gaps.locator(".rr-meta__i", { hasText: "Flow gap" })).toContainText("0 pp");
...
test("devnet-no-recommendation on /allocation: Recommended and the governance gap are missing, the flow gap is not", async ({ page }) => {
  ...
  await setMode(page, "devnet-no-recommendation");
  await openAllocation(page);
  await expect(vaultCol(page, 2)).toHaveText(["—", "—", "—", "—"]);
  await expect(vaultCol(page, 5)).toHaveText(["—", "—", "—", "—"]);
  await expect(vaultRows(page).nth(3).locator("td").nth(5)).toHaveText("0 pp");
...
test("the detail failing leaves the overview's figures and names the gap where the detail would be", async ({ page }) => {
  ...
  await openVault(page, "rmusdc");
  await expect(page.locator("#holdings .rr-stat__v")).toHaveText("$72,000.00");
  for (const id of ["holdings", "history", "activity"]) {
    await expect(page.locator(`#${id} .rr-note`).first()).toHaveText("Vault detail unavailable");
  }
...
// its sessions and its base-mode book come from the shipped archive.
...
test("the vault subject on the devnet: the router and four vaults, one book grouped by vault", async ({ page }) => {
  ...
  await expect(hold.locator(".rr-stat__v")).toHaveText("$100,000.00");
  ...
  // The ring: each vault's actual weight beside the router's applied weight,
  // named as such. No delta: the latest recommendation gives the gap.
  const ring = hold.locator(".rr-legend__row");
  await expect(ring).toHaveCount(4);
  await expect(ring.locator(":scope > span")).toHaveText(SYMBOLS);
  await expect(ring.locator("small")).toHaveText(["applied 70%", "applied 10%", "applied 15%", "applied 5%"]);
  await expect(ring.locator(".alp__mv")).toHaveCount(0);
  await expect(ring.nth(0).locator("b")).toHaveText("72%");
...
test("the vault subject on Base: rmUSDC alone, against the weights in force", async ({ page }) => {
  ...
  // The book's date beside its total: the archive's reading is not the vault
  // page's snapshot.
  await expect(hold.locator(".rr-stat__sub")).toHaveText(/^Base · [A-Z][a-z]{2} \d{1,2}, 2026$/);
  ...
  const ring = hold.locator(".rr-legend__row");
  await expect(ring).toHaveCount(4);
  await expect(ring.nth(0).locator("b")).toHaveText("100%");
  await expect(ring.nth(0).locator("small")).toHaveText("target 95%");
  await expect(ring.nth(1).locator("small")).toHaveText("target 5%");
  ...
  // The positions by the names rmUSDC's page gives them, and no Notable list
  // restating the weights.
  await expect(hold.locator("tr.rr-in th")).toHaveText(["Gauntlet USDC Prime", "Aave V3 USDC", "Compound III USDC"]);
  await expect(hold).not.toContainText("Notable");

  await expect(hold.locator("tr.rr-group")).toHaveCount(1);
  await expect(hold.locator(".rr-area__legend li")).toHaveText(["rmUSDC", "Target"]);
  await expect(page.locator("[data-vault-label]")).toHaveCount(0);
...
async function covenantFindings(page: Page, root: string): Promise<string[]> {
  return page.evaluate(([sel, HUES]) => {
    ...
    const SQUARE = ".rr-dot, .rr-layers__bar i, .rr-area__pt, .rr-stat";
    const out: string[] = [];
...
for (const mode of ["devnet", "base"] as const) {
  test(`each vault keeps one hue on /allocation, its page and the vault subject (${mode})`, async ({ page }) => {
    ...
    expect(new Set(await paintOf(page, ".cv--detail .rr-layers__bar i, .rr-holdings .rr-share i", "backgroundColor")))
...
for (const width of [1440, 390]) {
  for (const mode of ["base", "devnet"] as const) {
    test(`at ${width}px (${mode}): /allocation, /vault/rmusdc and the vault subject, no errors and no sideways scroll`, async ({ page }) => {
      ...
      // On a phone the Vaults table fits: TVL and the two gaps give way to the
      // three layers (devnet), or TVL to the one gap (Base). Nothing hides
      // behind a sideways scroll.
      if (width === 390) {
        await expect(page.locator("#vaults thead th:visible")).toHaveText(
          mode === "devnet" ? ["Vault", "Recommended", "Applied", "Actual"] : ["Vault", "Recommended", "Actual", "Gap"]);
        const [sw, cw] = await page.locator("#vaults .rr-tablewrap").evaluate((el) => [el.scrollWidth, el.clientWidth]);
        expect(sw).toBeLessThanOrEqual(cw);
      }

      await navigate(page, "/vault/rmusdc");
      await expect(page.locator("#holdings .rr-stat__v")).toHaveText(mode === "devnet" ? "$72,000.00" : usd2(SAVED_BASE.tvlUsd));
