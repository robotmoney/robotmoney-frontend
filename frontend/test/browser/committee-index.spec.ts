...
// Issue #357: GET /api/committee/sessions now carries a slim regimeSummary
// (regime + composite, no history) per session, sourced from the exact same
// computation the detail endpoint serializes. views/committee.html has
...
// why the committee index read "PUBLISHED" on every card instead of a regime
...
// text against) and checks the shipped Alpine view (committee.js's
// regimeLabel()) actually renders the label instead of the fallback.

const json = (body: unknown) => ({ status: 200, contentType: "application/json", body: JSON.stringify(body) });

test("committee index renders the regime label for a session carrying regimeSummary, not the state fallback", async ({ page }) => {
  await page.route("**/api/committee/members*", (route) =>
  ...
  const withRegime = {
    ...
    committeeRecommendation: { quorum: { active: 3, submitted: 3, absent: 0, participation: 1 }, stances: {}, meanConfidence: 0.5, absent: [], type: "position_actions", consensus: [], disagreements: [] },
    socialDraftId: null,
  ...
  await page.route("**/api/committee/sessions*", (route) =>
    route.fulfill(json({ sessions: [withRegime, withoutRegime], nextCursor: null })));

  await page.goto("/committee");
  // committee.html's error paragraph is x-show (always in the DOM, toggled by
  // CSS), unlike the x-if error elements on subject/session/member/take views
  // — assert hidden, not absent.
  await expect(page.locator(".cv__error")).toBeHidden();

  const cards = page.locator(".cv__session-card");
  ...
  const regimeCard = page.locator('a.cv__session-card[href="/committee/sessions/sess-with-regime"]');
  await expect(regimeCard).toHaveCount(1);
  const regimeSpan = regimeCard.locator(".cv__session-meta > span").first();
  ...
  const regimeCardStateFallback = regimeCard.locator(".cv__session-meta > span").nth(1);
  ...
  const fallbackCard = page.locator('a.cv__session-card[href="/committee/sessions/sess-without-regime"]');
  await expect(fallbackCard).toHaveCount(1);
  const fallbackRegimeSpan = fallbackCard.locator(".cv__session-meta > span").first();
  await expect(fallbackRegimeSpan).toBeHidden();
  const fallbackStateSpan = fallbackCard.locator(".cv__session-meta > span").nth(1);
