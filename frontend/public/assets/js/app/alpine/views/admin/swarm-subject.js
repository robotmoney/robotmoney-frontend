...
// see backend/src/api/routes/swarm-admin.ts) — this page fetches the full
...
export function registerAdminSwarmSubject(Alpine) {
  Alpine.data("adminSwarmSubject", () => ({
    ...adminAuthState(),
    loading: false,
    error: null,
    subjectId: null,
    ...
    async submitEdit() {
      ...
      try {
        ...
        await api.adminPost(
          path(ROUTES.swarm.admin.subjectUpdate, { id: this.subjectId }),
          this._token(),
          body,
        );
        this.editing = false;
    ...
    async confirmDeactivate() {
      ...
      try {
        await api.adminPost(
          path(ROUTES.swarm.admin.subjectDeactivate, { id: this.subjectId }),
          this._token(),
