...
export function registerViews(Alpine) {
  ...
  Alpine.data("walletPerfView", () => ({
    ...
    aumSeries: [
      { label: "Stable (USDC, ZYFAI-SS1)", color: SERIES.emerald, data: [0, 6460, 10020, 8720, 13080, 14530, 16340, 13970] },
      { label: "Protocol (WETH, ETH)", color: PALETTE.warm, data: [21790, 22240, 27570, 20340, 27890, 29960, 29960, 24270] },
      { label: "Agent (ROBOTMONEY, BNKR)", color: PALETTE.blue, data: [50850, 43040, 45950, 29060, 41840, 40860, 39040, 30890] },
      { label: "Stocks (SP500)", color: PALETTE.purple, data: [0, 0, 0, 0, 4360, 5450, 5450, 4410] },
    ],
    pctSeries: [
      { label: "Stable (USDC, ZYFAI-SS1)", color: SERIES.emerald, data: [0, 9, 12, 15, 15, 16, 18, 19] },
      { label: "Protocol (WETH, ETH)", color: PALETTE.warm, data: [30, 31, 33, 35, 32, 33, 33, 33] },
      { label: "Agent (ROBOTMONEY, BNKR)", color: PALETTE.blue, data: [70, 60, 55, 50, 48, 45, 43, 42] },
      { label: "Stocks (SP500)", color: PALETTE.purple, data: [0, 0, 0, 0, 5, 6, 6, 6] },
    ],
      ...
      const instance = new window.Chart(canvas, {
        ...
        data: {
          ...
          datasets: series.map((s) => ({
            ...
            backgroundColor: rgba(s.color, 0.45),
        ...
        options: {
          ...
          interaction: { mode: "index", intersect: false },
          scales: {
            y: { stacked: true, beginAtZero: true, max, grid: { color: GRID_COLOR }, ticks: { color: PALETTE.textMuted, callback: tick } },
            x: { grid: { display: false }, ticks: { color: PALETTE.textDim, maxTicksLimit: 8 } },
          },
          plugins: { legend: { display: false } },
        },
