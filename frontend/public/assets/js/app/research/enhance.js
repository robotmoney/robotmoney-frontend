...
import { initConceptTooltips } from "../lib/tooltip.js";
import { mountAllocationExplorers } from "../components/allocation-explorer.js";
export function enhanceResearch(scope = document) {
const cleanupTips = initConceptTooltips(scope);
mountAllocationExplorers(scope);
...
const lists = [];
for (const root of scope.querySelectorAll("[data-list]:not([data-remote-list])")) {
...
return () => { cleanupTips(); window.removeEventListener("hashchange", revealHash); };
}
