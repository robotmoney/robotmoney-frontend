...
import { allocationExplorer } from "../components/allocation-explorer.js";
import {
  esc as e,
  sleeves,
  num,
...
export const sessionPath = (r) =>
  r.mode === "live" ? `/swarm/sessions/${encodeURIComponent(r.id)}` : r.mode === "stress"
...
const changed = (r) =>
  r.reference && r.weights
    ...
    : null;
const outcome = (r) =>
  !r.weights ? (r.state === "published" || r.state === "cancelled" ? "Recommendation unavailable" : "Recommendation pending") : changed(r) == null
    ? "Reference unavailable"
    : changed(r)
      ? `${changed(r)} sleeve changes proposed`
      : "Reference weights retained";
const timing = (r) => {
  const stamp = r.mode === "stress" ? r.date : r.mode === "live" ? r.publishedAt || r.generatedAt : null;
  return stamp && Number.isFinite(Date.parse(stamp)) ? ` · ${new Date(stamp).toISOString().slice(11, 16)} UTC` : "";
};
const conviction = (t) =>
...
const wrap = (mode, content) =>
  `${mode === "live" ? "" : provenance(mode)}<article class="rr${mode === "live" ? " rr--live" : ""}">${content}<aside class="rr-disclaimer" aria-label="Swarm disclaimer"><p class="rr-eyebrow">Disclaimer</p><p>${e(SWARM_DISCLAIMER)}</p></aside></article>`;
const bread = (items) =>
...
const referenceNote = (r) =>
  `<p class="rr-note">${r.briefUnavailable ? "Session inputs are temporarily unavailable. Try again to load the reference policy." : r.reference ? `Reference weights are from the brief supplied to this session, policy dated ${date(r.referenceAsOf)}. Changes are percentage points.` : "Session-time reference weights are unavailable. No change from current policy is inferred."}</p>`;
const pager = () =>
...
export function historyRow(r) {
  return `<tr data-item data-search="${e([r.date, r.id, r.rationale].join(" ").toLowerCase())}" data-kind="${changed(r) == null ? "unknown" : changed(r) ? "changed" : "retained"}"><th scope="row"><a class="rr-rowlink" href="${sessionPath(r)}">${date(r.date)}<span aria-hidden="true"> ↗</span></a>${timing(r) ? `<small>${timing(r).slice(3)}</small>` : ""}<small>${r.takeCount ?? r.takes.length} analyst takes</small></th>${sleeves.map((s, i) => `<td><span class="rr-mobile-label">${s.short}</span>${percent(r.weights?.[i])}</td>`).join("")}<td class="rr-history-outcome">${r.reference && r.weights ? r.weights.map((v, i) => (Math.abs(v - r.reference[i]) > 0.005 ? `<span>${e(sleeves[i].short)} ${delta(v, r.reference[i])}</span>` : "")).join("") || "<span>Reference retained</span>" : !r.weights ? "Recommendation unavailable" : "Reference unavailable"}</td></tr>`;
}
export function subjectPage(records, mode, { latest = records[0] } = {}) {
  const r = latest,
...
export function sessionPage(r, records) {
  ...
  return wrap(
    mode,
    `${bread([["Swarm", "/swarm"], ["Allocation research", subjectPath + query(mode)], [date(r.date)]])}${title(`Flagship allocation · ${e(r.state || "published")} session`, "Allocation review", `${date(r.date)}${timing(r)}`)}<div class="rr-meta"><span>Analyst takes <b>${r.takes.length}</b></span><span>Disagreement topics <b>${r.disagreements.length}</b></span><span>Execution <b>Not reported</b></span><a href="${mode === "live" ? r.source : jsonLink(mode)}" type="application/json" target="_blank" rel="noopener">Session JSON ↗</a></div>${sourceNote(mode)}
...
<section class="rr-section" id="evidence">${section("04", "Evidence & provenance")}<div class="rr-evidence"><div><h3>${mode === "live" ? "Session record" : mode === "stress" ? "Scale-test fixture" : "Archived, unsigned research"}</h3><p>${mode === "live" ? "Each take identifies its signature status. Archived contributions are preserved as unsigned research." : mode === "stress" ? "Dates, analyst identities and vectors are synthetic. Original archive passages are deliberately replayed to test long content." : "These takes predate member key registration. They were never member-signed; this is not a failed signature check."}</p><p>Application of this recommendation is not confirmed. A verified research receipt does not establish execution.</p></div><dl>${mode === "live" ? `<div><dt>Consensus receipt</dt><dd>${e(r.receiptStatus || "Unavailable")}${r.receiptStatus === "Verified" || r.receiptStatus === "Not verified" ? ` · <a href="${e(r.receiptSource)}" target="_blank" rel="noopener">Read receipt ↗</a>` : ""}</dd></div>` : ""}<div><dt>Record generated</dt><dd>${e(r.generatedAt || "Unavailable")}</dd></div><div><dt>Reference policy</dt><dd>${r.referenceAsOf ? date(r.referenceAsOf) : "Not available for this session"}</dd></div><div><dt>Original source</dt><dd><a href="${e(r.source)}" target="_blank" rel="noopener">Original session JSON ↗</a></dd></div>${r.brief ? `<div><dt>Session inputs</dt><dd><a href="${e(r.brief)}" target="_blank" rel="noopener">Original brief JSON ↗</a></dd></div>` : ""}<div><dt>Readable data</dt><dd><a href="${mode === "live" ? r.source : jsonLink(mode)}" target="_blank" rel="noopener">Complete session JSON ↗</a></dd></div></dl></div></section>
<nav class="rr-record-nav" aria-label="Session navigation">${older ? `<a href="${sessionPath(older)}">← ${date(older.date)}${timing(older)}</a>` : "<span></span>"}<a href="${subjectPath + query(mode)}#history">All allocation sessions</a>${newer ? `<a href="${sessionPath(newer)}">${date(newer.date)}${timing(newer)} →</a>` : "<span></span>"}</nav>`,
  );
}
