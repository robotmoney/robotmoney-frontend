...
`contract/src/committee-application.js`) and the participation guide at
`<host>/views/docs/investment-committee/participation.html` document the same bytes as
...
`<host>/views/docs/investment-committee/participation.html` and the `contract` package's
