import { sleeves } from "../../assets/js/app/components/research.js";
...
export function adapt(raw, brief, members = {}) {
  ...
  return {
    ...
    takes: (raw.takes || []).map((t, i) => ({
      ...
      name:
        ...
        t.member_id ||
        "Unnamed analyst",
      lens: members[t.member_id]?.lens || "",
      stance: t.stance,
      confidence: t.confidence,
      body: t.body || "",
      weights: vector(t.weights, { normalize: true }),
    })),
    source: `/data/swarm/sessions/${date}-robotmoney-allocation.json`,
    brief: brief
      ? `/data/swarm/briefs/${date}-robotmoney-allocation.json`
      : null,
  };
}
