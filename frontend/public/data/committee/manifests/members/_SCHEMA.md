...
```jsonc
{
  ...
  "voice_doc": "data/committee/members/<id>.voice.md",  // optional; file injected verbatim into system prompt
  "self_advocacy_prompt": "<paragraph>",   // optional override for how this persona defends itself
  "api_key_hash": "sha256:...",            // set by scripts/committee/activate-member.js;
  ...
  "submit": {                              // required iff mode != pull
    ...
    "submission_path": "data/committee/submissions/<date>-<subject_id>/<member_id>.json",
  ...
  "wallet": {                              // optional, if this persona maps to a tracked subject
    "subject_id": "<id-in-subjects/>"      // FK into data/committee/subjects/
  },

  "avatar": {                              // optional; if absent, UI falls back to colored initials SVG
    "path": "/avatars/committee/<id>.<ext>",  // public URL path; render destination
...
```
node scripts/committee/sync-avatars.js                # only downloads missing
node scripts/committee/sync-avatars.js --force        # re-downloads all
node scripts/committee/sync-avatars.js --member woon  # one member
```
If `source_url` is null the avatar is treated as manually maintained — commit
the file directly under `public/avatars/committee/`. Athena's placeholder SVG
...
- **submit** — Agent runs its own infra and pushes the take to the
  IC. Two transports:
    - **git** — write `data/committee/submissions/<date>-<subject>/<member>.json`
      and commit. Used by RobotMoney via OpenClaw. No HTTP exposure
      needed.
    - **http** — POST signed payload to `/api/committee/submit`.
...
`scripts/committee/lib/manifests.js` validates every manifest on load.
