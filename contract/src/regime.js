...
// 0.33/0.67); every other surface that labels a composite — the swarm
// domain layer (backend/src/swarm/domain.ts) and the swarm memo builder
// (scripts/lib/swarm/memo.ts) — MUST consume this module rather than re-deriving its own
// thresholds. Pure data + one pure function, zero runtime deps, so both the
// backend and the swarm tooling can share it.
//
