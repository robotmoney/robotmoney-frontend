...
//
// What it exists to catch: COMMITTEE_ONBOARDING_SKILL_URL is the single
...
import { COMMITTEE_ONBOARDING_SKILL_URL } from "../../src/committee-application.js";

const TIMEOUT_MS = 30_000;

describe("COMMITTEE_ONBOARDING_SKILL_URL — live reachability", () => {
  test(
    "serves HTTP 200 and the committee-onboarding skill's own content",
    async () => {
      const res = await fetch(COMMITTEE_ONBOARDING_SKILL_URL, { redirect: "follow" });
      ...
      expect(body).toContain("name: committee-onboarding");
