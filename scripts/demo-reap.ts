...
// over an hour and broke the operator's demo, and the host had by then
// accumulated stale containers up to four days old. The full root-cause writeup
// and the three guards live in scripts/lib/demo-reap.ts's header.
//
...
//
//   bun run demo:reap -- --dry-run                    # see the plan, change nothing
//   bun run demo:reap -- --dry-run --older-than 30m   # widen the window, still read-only
//   bun run demo:reap                                  # reap, default threshold 6h
//   bun run demo:reap -- --env-class ci                # CI leftovers only (what e2e.yml runs)
//
...
import { makeDockerRunner } from "./lib/demo-volumes.ts";
...
try {
  ...
  console.error(`[demo:reap] ${err instanceof Error ? err.message : err}`);
  process.exit(2);
}

const envClassRaw = (flagValue("--env-class") ?? "all").trim();
if (envClassRaw !== "ci" && envClassRaw !== "local" && envClassRaw !== "all") {
  console.error(`[demo:reap] invalid --env-class ${JSON.stringify(envClassRaw)} — expected ci | local | all`);
  process.exit(2);
}
...
console.log(`[demo:reap] ${dryRun ? "DRY RUN — nothing will be removed" : "REAPING"}`);
console.log(`[demo:reap]   scope:      robotmoney.env=${envClass === "all" ? "<any> (label present)" : envClass}`);
console.log(`[demo:reap]   threshold:  older than ${formatAge(olderThanMs)} (--older-than ${olderThanRaw})`);
console.log(`[demo:reap]   ${active.note}`);
if (selfEnvHash) console.log(`[demo:reap]   G3 protects this job's own environment (robotmoney.env.hash=${selfEnvHash})`);
...
console.log(`[demo:reap] considered ${containers.length} labelled container(s): ${plan.doomed.length} to remove, ${kept.length} kept.`);
if (kept.length) {
...
if (outcome.removedContainers.length) {
  console.log(`[demo:reap] ${dryRun ? "would remove" : "removed"} ${outcome.removedContainers.length} container(s): ${outcome.removedContainers.join(", ")}`);
}
if (outcome.removedNetworks.length) {
  console.log(`[demo:reap] ${dryRun ? "would remove" : "removed"} ${outcome.removedNetworks.length} compose network(s): ${outcome.removedNetworks.join(", ")}`);
}
if (outcome.removedVolumes.length) {
  console.log(`[demo:reap] ${dryRun ? "would reclaim" : "reclaimed"} ${outcome.removedVolumes.length} volume(s): ${outcome.removedVolumes.join(", ")}`);
}
if (!outcome.removedContainers.length && !outcome.removedNetworks.length && !outcome.removedVolumes.length) {
  console.log("[demo:reap] nothing to reap — every candidate was protected by a guard or is younger than the threshold.");
}

let failed = 0;
for (const f of outcome.failedContainers) {
  console.error(`[demo:reap] FAILED to remove container ${f.name} — ${f.reason}`);
  failed++;
}
for (const f of outcome.failedNetworks) {
  console.error(`[demo:reap] FAILED to remove network ${f.name} — ${f.reason}`);
  failed++;
}
for (const s of outcome.skippedVolumes) {
  console.error(`[demo:reap] FAILED to reclaim volume ${s.name} — ${s.reason}`);
  failed++;
}
if (failed > 0) {
  console.error(`[demo:reap] ${failed} removal(s) were attempted and did NOT succeed — the leak survives. Exiting non-zero.`);
  process.exit(1);
}

if (dryRun) console.log("[demo:reap] dry run complete — nothing was changed. Re-run without --dry-run to apply.");
process.exit(0);
