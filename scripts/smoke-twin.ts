...
// public smoke backed by production-shaped data instead of simulation fixtures.
//
...
//     not run this.
//   * The smoke-twin's volume OUTLIVES the boot (the ephemeral-pgdata contract), so a
//     standing smoke-twin leaves production-derived data — password hashes, session
//     tokens, member emails — resident on this host until `smoke:clean`.
//
...
if (import.meta.main) {
  ...
  console.warn(
    `[${NAME}] ############################################################\n` +
      ...
      `[${NAME}] #\n` +
      `[${NAME}] # The smoke-twin's volume OUTLIVES this boot. Reclaim it when done:\n` +
