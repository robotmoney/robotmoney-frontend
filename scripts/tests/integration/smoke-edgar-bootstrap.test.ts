...
//      CLI (the exact command scripts/lib/smoke-main.ts runs after migrations
...
//   2. structurally — scripts/lib/smoke-main.ts (which spawns docker
...
// this ROOT-suite test (cross-package tsc coupling; see smoke-main.ts's own
...
// ── structural: smoke-main.ts wires the CLI after migrations+API health, before CI checks ──

test("scripts/lib/smoke-main.ts runs the isolated producer seed command AFTER API health and BEFORE the CI checks / local action loops", () => {
