...
async function runSmoke(backend: ReturnType<typeof startStubBackend>, env: Record<string, string> = {}) {
  const proc = Bun.spawn(["bun", "run", "scripts/smoke-live-smoke.ts"], {
    cwd: repoRoot,
    env: {
      ...process.env,
...
describe("smoke-live-smoke (nightly LIVE gate self-test, issue #128)", () => {
...
// the standing smoke moved to a 6 h swarm interval, deriving from "the
...
describe("LIVE_SMOKE_DEADLINE_MS is bounded and pinned to the fast/CI profile", () => {
  ...
  test("the script itself carries no cadence literal — it derives, per issue #128", () => {
    const src = readFileSync(join(repoRoot, "scripts", "smoke-live-smoke.ts"), "utf8");
    expect(src).toContain('resolveSmokeCadence({ stage: false })');
