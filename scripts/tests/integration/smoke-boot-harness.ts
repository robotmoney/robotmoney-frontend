...
export function harness(prefix: string): BootHarness {
  const root = mkdtempSync(join(tmpdir(), `rm-${prefix}-`));
  ...
  const emptyRoster = join(root, "credential.json");
...
export function bootArgs(h: BootHarness, extra: readonly string[] = []): string[] {
  return ["bun", "--no-env-file", "scripts/smoke.ts", "--local", "blank", "--migrate", "--instance", h.instance, ...extra];
}
 ...
 */
export function spawnBoot(h: BootHarness, extra: readonly string[] = [], opts: { tty?: boolean } = {}): RunningBoot {
  const argv = bootArgs(h, extra.length > 0 ? extra : ["--credentials", h.emptyRoster]);
