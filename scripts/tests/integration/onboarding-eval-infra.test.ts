...
// Releases (the `rmpc` binary, same as scripts/tests/integration/rmpc-canonical-apply.test.ts),
// and network egress to pull/build base images are all hard dependencies of
// the describe block below — same policy as
// scripts/tests/integration/demo-compose-config.test.ts. A missing docker CLI throws in
