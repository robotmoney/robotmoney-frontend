...
function startStubBackend(o: StubOverrides = {}) {
  ...
  return Bun.serve({
    ...
    fetch(req) {
      ...
      if (p === ROUTES.committee.sessions) return Response.json({ sessions });
      for (const s of sessions) {
        const sessPath = routePath(ROUTES.committee.session, { date: s.date, subject: s.subjectId });
        if (p === sessPath) {
      ...
      if (p === ROUTES.committee.members) return Response.json({ members: o.members ?? REAL_MEMBERS });
...
describe("verify-cutover-data-gate (issue #344 operator-script self-test)", () => {
  test("healthy gate, with admin token → exit 0, every check reported pass", async () => {
    ...
    try {
      ...
      expect(r.output).toContain("✓ committee sessions resolve");
      expect(r.output).toContain("✓ committee member roster");
