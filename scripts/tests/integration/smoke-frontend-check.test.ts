...
//
// scripts/smoke-frontend-check.ts only ever runs against a live backend (docker-compose
// smoke stack or `bun run smoke`), which this "root" test job (`bun run test`, wired in
...
async function runCheck(backend: ReturnType<typeof startStubBackend>) {
  const proc = Bun.spawn(["bun", "run", "scripts/smoke-frontend-check.ts"], {
    cwd: repoRoot,
...
describe("scripts/smoke-frontend-check.ts (smoke readiness gate self-test)", () => {
