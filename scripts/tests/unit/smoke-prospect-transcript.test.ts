// Issue #317: the standing live smoke must retain and expose a discoverable,
...
// combined transcript to the shared smoke log when a prospect fails.
//
// scripts/lib/smoke-main.ts cannot be imported in a unit test (it boots a
// stack at module scope — see scripts/tests/unit/smoke-onboarding-driver.test.ts's
// doc comment), so the WIRING into smoke-main.ts is graded over source text
// there. This file proves the actual BEHAVIOUR of the extracted primitive
// smoke-main.ts wires in: scripts/lib/smoke-prospect-transcript.ts. No Docker,
...
import { buildOutcomeRecord, startProspectTranscript } from "../../lib/smoke-prospect-transcript.ts";
...
describe("startProspectTranscript (scripts/lib/smoke-prospect-transcript.ts)", () => {
  ...
  test("an event emitted through the sink is durably readable on disk immediately — proves the in-progress tailing path", () => {
    ...
    try {
      ...
      expect(events[0].composeProject).toBe("rm_smoke_abc123");
  ...
  test("two attempts sharing one sink land in the SAME directory, each correctly tagged with its own attempt number — the retry case", () => {
    ...
    try {
      ...
      const attempt1 = createOnboardingTelemetry({ composeProject: "rm_smoke_abc123", runId: "helios", attempt: 1 }, transcript.sink);
      attempt1.emit({ source: "harness", stream: "event", message: "attempt 1 refused" });
      const attempt2 = createOnboardingTelemetry({ composeProject: "rm_smoke_abc123", runId: "helios-r2", attempt: 2 }, transcript.sink);
  ...
  test("finish() writes a result.json with the classified outcome, branch, reason, and correlating fields — no Docker required", () => {
    ...
    try {
      ...
      expect(result.smokeRun).toBe("rm_smoke_abc123");
...
describe("buildOutcomeRecord (pure)", () => {
  test("computes the SAME outcome classifyOutcome/explainOutcome would, from a synthetic result — no Docker, no inference", () => {
    ...
    const record = buildOutcomeRecord("rm_smoke_xyz", result, 1234);
    expect(record.smokeRun).toBe("rm_smoke_xyz");
