// EXECUTED proof of `bun run smoke:clean`'s exit-code contract
// (scripts/smoke-clean.ts) — the loud-skip-never-silent-skip invariant applied to
// teardown.
//
// WHY THIS FILE EXISTS. e2e run 30406428674 was cancelled mid-boot, so the smoke
// boot step's own `docker compose down` never ran and the stack survived. The
// `if: always()` backstop then ran smoke-clean, which removes VOLUMES only: it
// found the run's pgdata volume still referenced by a live container, printed
//
//     [smoke:clean] SKIPPED 1 volume(s) (left intact):
//       ⚠ rmsmoke_ci_30406428674_1_pgdata — in use ... [676c43de5091...]
//
...
const CLEAN = join(repoRoot, "scripts", "smoke-clean.ts");

// A `docker` shim covering exactly the three calls smoke-clean makes:
...
case "$1 \${2:-}" in
  ...
  "volume ls")
    ...
    echo '{"Name":"rm_ci_stack_30406428674_1_pgdata","Labels":"robotmoney.smoke=1,robotmoney.smoke.project=rm_ci_stack_30406428674_1","Driver":"local","Scope":"local"}'
    exit 0
    ;;
  "volume rm")
...
function runClean(args: string[], env: Record<string, string> = {}): RunResult {
  const dir = mkdtempSync(join(tmpdir(), "smoke-clean-fakedocker-"));
  try {
...
describe("smoke:clean exit codes — the CI backstop role (--project)", () => {
  ...
  test("the failure is ACTIONABLE — it names the volume, the holding container, and the command that clears it", () => {
    ...
    expect(r.output).toContain("smoke:reap");
  });
...
describe("smoke:clean exit codes — the operator role (no --project)", () => {
  test("an in-use volume exits 0 — bare `bun run smoke:clean` while your smoke is up is not an error", () => {
    ...
    expect(r.output).toContain("676c43de5091abcdef");
    expect(r.output).toContain("bun run smoke:down");
    expect(r.output).not.toContain("TEARDOWN INCOMPLETE");
  });
...
describe("smoke:clean exit codes — a dead daemon is red in BOTH roles", () => {
  test("with --project", () => {
