// PURE, per-PR coverage for the admission-eval scorecard (docs/architecture.md
// §11.3 E4) and the shared outcome classification it scores.
//
...
// place a scoring bug could surface would be a multi-hour sweep — and a
// scorecard that mis-scores is worse than no scorecard, because it launders a
// red into a green.
//
...
// A past sweep once reported "admission rate 0.20" from a run in which the
// harness itself stopped the agent. These cases pin the two halves of the fix
// that must hold together: the broken samples leave the denominator (so the
// reported rate stops lying), AND their presence fails the sweep outright (so
...
describe("harness failures are classified as harness-error, not as a product outcome", () => {
  ...
  test("compose's interactive volume-recreate question in the run's output is harness-error", () => {
    const transcript = `--- stdout ---\nVolume "rm_ci_eval_x_pgdata" ${COMPOSE_VOLUME_RECREATE_PROMPT}. Recreate (data will be lost)?\n--- stderr ---\n`;
    expect(classifyOutcome(timedOutRun({ transcript }))).toBe("harness-error");
...
describe("run-log artifact honesty", () => {
  ...
  test("the header states what the bytes are, and reports whether the agent produced anything at all", () => {
    ...
    // The sentence that would have saved an hour on the original investigation.
...
describe("an admitted run is never re-labelled a harness failure", () => {
  test("explainOutcome keeps admitted, but STILL reports a fault — the hazard the consumer must guard", () => {
    ...
    // sweep. The admission sweep therefore records the fault only when it
    // decided the outcome. This assertion exists so that if explainOutcome is
  ...
  test("a sweep of admissions stays green even when one carries a quoted trigger string", () => {
    // Mirrors what the admission sampler now records: the fault is kept ONLY
    // when it decided the outcome, so scoring and the sweep gate agree.
