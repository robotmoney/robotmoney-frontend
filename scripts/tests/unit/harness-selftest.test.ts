...
// ── eval:onboarding:isolated selector (issue #279) — no model call involved ──
//
// `eval:onboarding:isolated` (package.json) is, underneath its two-process
// runtime-first ordering (see evals/README.md), the SAME `bun test` runner
// mechanism as test:unit/test:integration above, just pointed at
// evals/onboarding/isolated/ with `evals/` as its working directory. This
// proves the selector inherits the two harness properties every other Bun
// selector in this repo must have — an empty selection is RED and a
// collection-time failure is RED — against the SAME shared fixture trees,
// with ZERO Docker and ZERO model inference: only bun's own collection/exit-
// code behaviour is under test here, exactly as for test:unit/test:integration
// above.
describe("eval:onboarding:isolated selector (bun test, evals/ as cwd)", () => {
  const evalsDir = join(repoRoot, "evals");
  const fixturesFromEvals = join("..", "scripts", "tests", "fixtures", "harness", "bun-runner");

  function spawnFromEvals(argv: string[]): { code: number; out: string } {
    const r = Bun.spawnSync(argv, { cwd: evalsDir, stdout: "pipe", stderr: "pipe" });
    return { code: r.exitCode ?? -1, out: decode(r.stdout) + decode(r.stderr) };
  }

  test("package.json's eval:onboarding:isolated target is `cd evals && ... bun test ...` — the SAME runner mechanism", () => {
    const script = pkg.scripts["eval:onboarding:isolated"]!;
    expect(script).toMatch(/^cd evals && /);
    expect(script).toMatch(/\bbun test\b/);
  });

  test("an empty test selection exits nonzero — 0 tests collected can never be green, no model call involved", () => {
    const r = spawnFromEvals(["bun", "test", join(fixturesFromEvals, "empty-selection")]);
    expect(r.code).not.toBe(0);
  });

  test("a planted collection-time-failing fixture exits nonzero, no model call involved", () => {
    const r = spawnFromEvals(["bun", "test", "./" + join(fixturesFromEvals, "planted-fail.fixture.ts")]);
    expect(r.code).not.toBe(0);
    expect(r.out).toContain("1 fail");
  });

  test("a planted passing fixture exits 0 — the control", () => {
    const r = spawnFromEvals(["bun", "test", "./" + join(fixturesFromEvals, "planted-pass.fixture.ts")]);
    expect(r.code).toBe(0);
    expect(r.out).toContain("1 pass");
  });
});
