// Unit tests for the label-driven container reaper (scripts/lib/smoke-reap.ts) —
...
import type { DockerRunner } from "../../lib/smoke-volumes.ts";
...
function psLine(o: {
  ...
  const labels: string[] = [];
  if (o.project !== null) labels.push(`robotmoney.smoke.project=${o.project ?? "p"}`);
  if (o.envClass !== null) labels.push(`robotmoney.env=${o.envClass ?? "ci"}`);
  labels.push(`robotmoney.env.hash=${o.envHash ?? "aaaaaaaaaa"}`);
  if (o.role) labels.push(`robotmoney.smoke.role=${o.role}`);
...
function networkInspectJson(value: ReapNetwork): string {
  ...
  if (value.project !== null) labels["robotmoney.smoke.project"] = value.project;
...
describe("planReap — G1 the active smoke project is never touched", () => {
  const ancient = new Date("2026-07-01T00:00:00Z");

  test("refuses the project named by .agents/smoke-state.json, however old", () => {
    const cs = [
      container({ id: "api", name: "rm_smoke_stack_live-api-1", project: "rm_smoke_stack_live", envClass: "local", createdAt: ancient }),
      container({ id: "pg", name: "rm_smoke_stack_live-postgres-1", project: "rm_smoke_stack_live", envClass: "local", createdAt: ancient }),
    ];
    const plan = planReap(cs, { now: NOW, olderThanMs: SIX_HOURS, activeProjects: ["rm_smoke_stack_live"] });
    expect(plan.doomed).toEqual([]);
    // The guard protects the WHOLE project, not just the container that matched.
    for (const v of plan.verdicts) expect(v.reason).toMatch(/G1 active smoke project/);
  });

  test("but a DIFFERENT old project in the same sweep is still reaped (the guard is not a blanket no-op)", () => {
    const cs = [
      container({ id: "live", name: "live-api-1", project: "rm_smoke_stack_live", envClass: "local", createdAt: ancient, running: true }),
      container({ id: "stale", name: "stale-api-1", project: "rm_smoke_stack_stale", envClass: "local", createdAt: ancient }),
    ];
    const plan = planReap(cs, { now: NOW, olderThanMs: SIX_HOURS, activeProjects: ["rm_smoke_stack_live"] });
...
describe("planReap — G2 a live, serving project is never touched even when the state file disagrees", () => {
  ...
  test("a healthcheck-healthy local project survives with NO active-project entry at all (the state file was stale on this host)", () => {
    const cs = [
      container({ id: "api", name: "rm_smoke_stack_serving-api-1", project: "rm_smoke_stack_serving", envClass: "local", createdAt: ancient, running: true, healthy: true, status: "Up 4 days (healthy)" }),
      container({ id: "pg", name: "rm_smoke_stack_serving-postgres-1", project: "rm_smoke_stack_serving", envClass: "local", createdAt: ancient }),
    ];
    // activeProjects deliberately names a DEAD project — exactly the stale-state
    // -file situation observed on 2026-07-29.
    const plan = planReap(cs, { now: NOW, olderThanMs: SIX_HOURS, activeProjects: ["rm_smoke_stack_dead"] });
    expect(plan.doomed).toEqual([]);
  ...
  test("publishing a host port counts as serving even without a healthcheck", () => {
    const cs = [container({ id: "api", name: "rm_smoke_stack_p-api-1", project: "rm_smoke_stack_p", envClass: "local", createdAt: ancient, running: true, publishesHostPort: true, status: "Up 4 days" })];
    const plan = planReap(cs, { now: NOW, olderThanMs: SIX_HOURS });
    expect(plan.doomed).toEqual([]);
  ...
  test("a leaked TWIN does not count as proof of life — it publishes a port and serves nobody", () => {
    // The smoke-twin (`--db smoke-twin`) is a raw `docker run` postgres holding a restored
    ...
    // have pinned the smoke-twin AND every dead container beside it, on every future
    // reap, forever — the leak protecting itself.
    const cs = [
      container({ id: "smoke-twin", name: "rm-restore-20260821T101500Z-a3f9c1", project: "rm_smoke_stack_gone", envClass: "local", createdAt: ancient, running: true, publishesHostPort: true, role: "smoke-twin", status: "Up 4 days" }),
      container({ id: "api", name: "rm_smoke_stack_gone-api-1", project: "rm_smoke_stack_gone", envClass: "local", createdAt: ancient, running: false }),
    ];
    const plan = planReap(cs, { now: NOW, olderThanMs: SIX_HOURS });
    expect(plan.doomed.map((c) => c.id).sort()).toEqual(["api", "smoke-twin"]);
  });

  test("the smoke-twin exemption is NARROW — a real serving container in the same project still shields it (planted control)", () => {
    // The control for the test above: if the exemption were written as "ignore
    // host ports entirely" rather than "ignore the smoke-twin", this project would be
    // reaped out from under a live smoke. It must not be.
    const cs = [
      container({ id: "smoke-twin", name: "rm-restore-x", project: "rm_smoke_stack_live", envClass: "local", createdAt: ancient, running: true, publishesHostPort: true, role: "smoke-twin", status: "Up 4 days" }),
      container({ id: "api", name: "rm_smoke_stack_live-api-1", project: "rm_smoke_stack_live", envClass: "local", createdAt: ancient, running: true, publishesHostPort: true, status: "Up 4 days" }),
    ];
    const plan = planReap(cs, { now: NOW, olderThanMs: SIX_HOURS });
    expect(plan.doomed).toEqual([]);
    for (const v of plan.verdicts) expect(v.reason).toMatch(/G2 project is live and serving/);
  ...
  test("the role label is read off the docker line, not just hand-built objects", () => {
    const parsed = parseContainerLine(psLine({ id: "t", name: "rm-restore-y", role: "smoke-twin", ports: "172.17.0.1:49155->5432/tcp" }));
    expect(parsed?.role).toBe("smoke-twin");
  ...
  test("a STOPPED old local project is still reaped — G2 protects the serving, not the merely present", () => {
    const cs = [container({ id: "x", name: "rm_smoke_stack_dead-api-1", project: "rm_smoke_stack_dead", envClass: "local", createdAt: ancient, running: false })];
    const plan = planReap(cs, { now: NOW, olderThanMs: SIX_HOURS });
...
describe("readActiveProjects", () => {
  test("reads the project out of a real state file", () => {
    const dir = mkdtempSync(join(tmpdir(), "smoke-reap-state-"));
    try {
      const f = join(dir, "smoke-state.json");
      writeFileSync(f, JSON.stringify({ project: "rm_smoke_stack_ab12", apiPort: 48787 }));
      const r = readActiveProjects(f);
      expect(r.projects).toEqual(["rm_smoke_stack_ab12"]);
      expect(r.note).toContain("G1 protects project=rm_smoke_stack_ab12");
    } finally {
  ...
  test("a missing file yields an empty list and SAYS SO — G1 silently protecting nothing is the failure mode", () => {
    const r = readActiveProjects(join(tmpdir(), "definitely-absent-smoke-state.json"));
...
describe("planReap — managed network-only orphans", () => {
  ...
  test("active-project and self-environment guards protect network-only projects", () => {
    ...
    expect(plan.doomedNetworks).toEqual([]);
    expect(plan.networkVerdicts[0]!.reason).toContain("G1 active smoke project");
  ...
  test("unlabelled and adversarially mismatched networks are never selected", () => {
    const cases = [
      ...
      network({ id: "wrong-name", name: "rm_smoke_stack_aaaaaaaaaa_default" }),
...
describe("executeReap", () => {
  ...
  test("removes containers, then their compose networks, then their now-unreferenced volumes — in that order", () => {
    const { run, calls } = fakeRunner({
      ...
      volumes: [JSON.stringify({ Name: `${oldProject}_pgdata`, Labels: `robotmoney.smoke=1,robotmoney.smoke.project=${oldProject}` })],
    });
  ...
  test("volume reclamation stays scoped to the reaped project's label", () => {
    ...
    expect(ls).toContain(`label=robotmoney.smoke.project=${oldProject}`);
  });
...
describe("the fix is wired into every workflow that boots a smoke stack", () => {
  ...
  test("every always() teardown brings the compose project DOWN before reclaiming volumes", () => {
    for (const w of workflows) {
      ...
      // on this shared runner would take the standing stage smoke with it.
      expect({ workflow: w, down: text.includes('docker compose -p "$SMOKE_PROJECT" \\') }).toEqual({ workflow: w, down: true });
      expect({ workflow: w, flags: text.includes("down -v --remove-orphans") }).toEqual({ workflow: w, flags: true });
      // Ordering: compose down must precede the smoke-clean verdict.
      const downAt = text.indexOf("down -v --remove-orphans");
      const cleanAt = text.indexOf("scripts/smoke-clean.ts --project");
  ...
  test("every workflow reaps prior runs' CI leftovers, scoped to robotmoney.env=ci", () => {
    for (const w of workflows) {
      const text = workflow(w);
      expect({ workflow: w, reaps: text.includes("scripts/smoke-reap.ts --env-class ci --older-than") }).toEqual({ workflow: w, reaps: true });
      // `--env-class local` in CI would put the standing stage smoke in scope.
      expect({ workflow: w, local: text.includes("smoke-reap.ts --env-class local") }).toEqual({ workflow: w, local: false });
    }
  });
