  ...
  if (t.includes(HARNESS_PERMISSION_REJECTION)) {
    return {
      ...
      detail:
        ...
        "opencode.json (scripts/agent/member-agent.ts's buildAgentOpencodeConfig) — the harness, not the product, stopped this run",
    };
  }
 ...
 * onboarding at all), or a bare `timed-out`. `navigation-failure` is the one
 * outcome that is a REAL result: the agent tried and did not get there, and
 * retrying it would soften the gate.
 *
 * `timed-out` is listed as retryable HERE because on the free tier "a call can
 * take minutes and occasionally returns nothing", so a bare deadline says
 * nothing about the product. Since D22 rule 1 was amended (2026-07-28) the eval
 * ordinarily runs a FUNDED, registry-selected model, where a timeout IS a real
 * result — so scripts/lib/onboarding-eval.ts's runOnboardingEvalWithRetry gates
 * that one outcome on `resolveModelConfig().keyless` before acting on this
 * verdict. That tier judgement deliberately stays with the caller that knows
 * the tier; this function stays a pure function of the outcome alone.
 *
