...
function secretDirPrefix(project: string): string {
  return `robotmoney-demo-${safeProject(project)}-secrets-`;
}

/** Provision one mode-0600 Docker-secret file outside the checkout. */
export function provisionDemoAnalyticsToken(project: string, token: string): string {
...
export function provisionDemoAnalyticsTokenAfterPreflight(
  project: string,
  token: string,
  preflight: () => void,
): string {
  preflight();
  return provisionDemoAnalyticsToken(project, token);
}
 ...
 * anywhere else.
 */
export function removeDemoAnalyticsToken(file: string, project: string): boolean {
