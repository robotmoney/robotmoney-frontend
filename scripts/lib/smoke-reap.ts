...
// mid-boot. The smoke boot step owns its own `docker compose down`, so killing it
// meant teardown never ran and the whole stack survived. The `if: always()`
// backstop step then ran `smoke:clean`, which removes VOLUMES only — it found the
// run's pgdata volume still referenced by a live container, LOUDLY SKIPPED it,
// and exited 0:
//
//     [smoke:clean] found 1 smoke volume(s):
//     [smoke:clean] SKIPPED 1 volume(s) (left intact):
//       ⚠ rmsmoke_ci_30406428674_1_pgdata — in use (a container still
...
// stage.robotmoney-labs.dev — for over an hour and broke the operator's smoke.
// Two structural gaps produced it:
//
//   1. Nothing ever ran `docker compose down` for the project outside the boot
//      process itself (fixed in the workflows + the smoke-clean exit code).
...
// live site is an outage waiting to happen: `rm_smoke_stack_*` (the standing
// stage smoke) and `rm_ci_stack_*` (a CI run) differ by a prefix that one sloppy
// glob erases.
//
// THREE GUARDS, applied before anything is removed (see planReap):
//
//   G1 ACTIVE PROJECT — the project named in .agents/smoke-state.json is never
...
// smoke by construction, and the e2e job's own ceiling is 105 minutes, so a
...
import { listSmokeVolumes, removeSmokeVolumes, type DockerRunner } from "./smoke-volumes.ts";
...
export interface ReapContainer {
  id: string;
  name: string;
  /** robotmoney.smoke.project — the compose project this belongs to. */
  project: string | null;
  ...
  /**
   * robotmoney.smoke.role — what this container does inside its project, or null
   * for the ordinary serving containers. Only `smoke-twin` is set today, and only G2
   * reads it: a smoke-twin publishes a host port without serving anything, so counting
...
export interface PlanOptions {
  now: Date;
  olderThanMs: number;
  /** G1: projects that must never be reaped (from .agents/smoke-state.json). */
...
// "k=v,k2=v2" string, same shape as `docker volume ls` (see smoke-volumes.ts).
...
/**
 * The project named by `.agents/smoke-state.json`, if any. Takes the path so a
 ...
 * an operator's smoke, which the caller must be told. It is also why G2 exists at
...
export function planReap(containers: ReapContainer[], opts: PlanOptions, networks: ReapNetwork[] = []): ReapPlan {
  ...
  for (const [, members] of groups) {
    ...
    // G1 — the active smoke, named by .agents/smoke-state.json.
    if (project && active.has(project)) {
      for (const c of members) {
        byId.set(c.id, { container: c, reap: false, reason: `G1 active smoke project (.agents/smoke-state.json names ${project})` });
      }
      continue;
    }
    ...
    if (!isCi) {
      // A smoke-twin is EXCLUDED from the liveness evidence, not from the reap. It
      // publishes a host port because the stack's containers dial it, never
      // because anything is being served — so a smoke-twin that outlived its stack
  ...
  const networkVerdicts: ReapNetworkVerdict[] = networks.map((network) => {
    ...
    if (active.has(network.project!)) {
      return { network, reap: false, reason: `G1 active smoke project (.agents/smoke-state.json names ${network.project})` };
    }
...
export function executeReap(run: DockerRunner, plan: ReapPlan, opts: { dryRun?: boolean } = {}): ReapOutcome {
  ...
  for (const project of plan.doomedProjects) {
    // Scoped to the reaped project's label — the same discipline smoke:clean uses,
    // so a co-tenant project's volume can never be swept up by a wide filter.
    let names: string[];
    try {
      names = listSmokeVolumes(run, { project }).map((v) => v.name);
    } catch (err) {
    ...
    const res = removeSmokeVolumes(run, names);
