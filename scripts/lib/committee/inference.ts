// Committee-member take authorship via a REAL language-model call. This module
// shells out to
//
...
// committee session forever. Resolved at call time (not
  ...
  try {
    ...
    throw new Error(
      `opencode inference unavailable: failed to spawn '${bin}' (${err instanceof Error ? err.message : String(err)}). ` +
        `Committee takes require a working opencode CLI; there is NO template fallback in this path.`,
    );
  }
...
// Author one committee member's take with a REAL opencode-zen call.
