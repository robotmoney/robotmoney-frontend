...
`rm_app` / `rm_worker` alone and needs no schema-owner credential — but it is
NOT an unconditional restart: `backend/scripts/schema-current.ts` still checks,
read-only, whether the schema is current, and refuses the boot instead of
serving stale data if it is not (`scripts/lib/smoke-external-migrate.ts`'s
`refuseIfSchemaBehind()`). Absent `--migrate` AND with a current schema, it is
a restart in every sense that matters.
