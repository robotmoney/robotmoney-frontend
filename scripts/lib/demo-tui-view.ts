// The standing demo's TUI STATE MACHINE (issue #456 — extracted out of the
// 2087-line scripts/lib/demo-main.ts, which the 2026-07-14 maintainability
...
// module-level mutable state of its own — so demo-main.ts (or a future unit
// test) owns exactly one DemoState instance and threads it through. That is
...
// arrived.
export function startOnboarding(state: DemoState, memberId: string, name: string): void {
...
export function setOnboardStep(state: DemoState, memberId: string, key: string, status: OnboardStepStatus): void {
