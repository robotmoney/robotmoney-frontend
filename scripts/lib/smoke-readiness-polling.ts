// The standing smoke's READINESS-PROBE POLLING concern (issue #456 — extracted
// out of scripts/lib/smoke-main.ts alongside the TUI state machine in
// smoke-tui-view.ts). Two independent polls:
//
...
// per smoke boot (or a fake one in a test) instead of this being process-wide.
...
export function createReadinessPolling(deps: ReadinessPollingDeps) {
  ...
  return {
    ...
    // Exposed for tests / diagnostics only — nothing in smoke-main.ts reads
