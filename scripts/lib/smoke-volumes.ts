// Shared smoke-volume discovery + deletion. Single source of truth for what
// counts as "one of the smoke's postgres volumes", used by BOTH the operator CLI
// (scripts/smoke-clean.ts) and the CI teardown path (scripts/lib/smoke-main.ts).
//
// Lifecycle context (issue: smoke persistent volumes): teardown no longer deletes
// the pgdata volume — `bun run smoke` (SIGTERM/Ctrl-C) and `bun run smoke:down`
// both run `docker compose down` WITHOUT `-v`, so smoke data survives a reboot.
// Deleting a smoke volume is now an EXPLICIT act, and this module is the only code
...
// non-smoke stack, but the label is applied only by our overlay.
//
...
export const SMOKE_VOLUME_LABEL = "robotmoney.smoke=1";
export const SMOKE_VOLUME_PROJECT_LABEL = "robotmoney.smoke.project";

export interface SmokeVolume {
  name: string;
  /** Value of the robotmoney.smoke.project label, if present. */
  project: string | null;
}
...
// the same daemon the smoke used is targeted.
...
export function parseVolumeLine(line: string): SmokeVolume | null {
  const t = line.trim();
  if (!t) return null;
  ...
  return { name: obj.Name, project: labels.get(SMOKE_VOLUME_PROJECT_LABEL) ?? null };
}

// List every smoke pgdata volume (label robotmoney.smoke=1). Pass a project to
// scope to a single run's volume (CI teardown deletes ONLY its own run's volume,
// never a co-tenant standing smoke's). A missing docker CLI / daemon fails loudly
// (throws) — never a silent empty list that would mask a leak.
export function listSmokeVolumes(run: DockerRunner, opts: { project?: string } = {}): SmokeVolume[] {
  const args = ["volume", "ls", "--filter", `label=${SMOKE_VOLUME_LABEL}`];
  if (opts.project) args.push("--filter", `label=${SMOKE_VOLUME_PROJECT_LABEL}=${opts.project}`);
...
export function removeSmokeVolumes(run: DockerRunner, names: string[]): CleanResult {
...
// These are spawned dynamically during smoke runs and must be force-removed before network/stack
...
// environment-scoped too: a CI teardown can never match the standing smoke's
...
// them" (`bun run smoke:clean` with no --project).
export function purgeSmokeEvalContainers(
  run: DockerRunner,
  opts: { project?: string } = {},
): CleanResult {
