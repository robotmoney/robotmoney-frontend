// Standing-demo newcomer roster (fixed, finite — never generated). Split out
// of demo-main.ts, mirroring demo-env.ts's split for resolveDemoEnv, so this
// pure logic is importable from tests without triggering demo-main.ts's
// module-load side effects (port allocation, log files, docker compose).
//
// The demo's periodic onboarding driver admits EXACTLY these named
