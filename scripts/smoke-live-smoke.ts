...
// and the hermetic smoke path entirely, so this script now runs on EVERY smoke
...
// cold-start wallet.sample_balances job — the standing smoke's explicitly seeded
 ...
 *
 * This gate only ever runs against a CI / `bun run smoke` stack, which is always
 * the fast profile — so it is pinned to that profile explicitly and NEVER to
 * whatever cadence the caller happens to be running. The standing smoke's
 ...
 */
export const LIVE_SMOKE_DEADLINE_MS = 2 * resolveSmokeCadence({ stage: false }).swarmIntervalMs;
...
// scripts/tests/integration/smoke-live-smoke.test.ts.
...
export async function collectFailures(backend: string): Promise<string[]> {
  const [sessions, regime, wallet, vault, ...research] = await Promise.all([
    ...
    // long-running smoke stack the newest 20 sessions across every state could
...
if (import.meta.url === `file://${process.argv[1]}`) {
  main().catch((e) => {
    console.error("smoke-live-smoke error:", e);
    process.exit(1);
  });
}
