...
// host:
//
//   bun scripts/verify-cutover-data-gate.ts --base https://<host>
//   ADMIN_TOKEN=... bun scripts/verify-cutover-data-gate.ts --base https://<host>
//
// <host> is whatever host is currently serving an api process pointed at the
// production database (see D29 for the cutover host). No specific hostname is
// baked in here on purpose: the stage tunnel this script was originally
...
//   2. Live surface resolution — every session the swarm API reports
...
export async function checkSessionsResolve(base: string): Promise<CheckResult> {
  const { status, body } = await getJson(`${base}${ROUTES.swarm.sessions}?full=1`);
  if (status !== 200 || !body || typeof body !== "object") {
    return { name: "swarm sessions resolve", status: "fail", detail: `GET ${ROUTES.swarm.sessions} -> ${status}` };
  }
  const sessions = (body as { sessions?: Array<{ date: string; subjectId: string }> }).sessions ?? [];
  if (sessions.length === 0) {
    return { name: "swarm sessions resolve", status: "fail", detail: "no sessions returned" };
  }
  const failing: string[] = [];
  for (const s of sessions) {
    const url = `${base}${routePath(ROUTES.swarm.session, { date: s.date, subject: s.subjectId })}`;
    const res = await fetch(url);
    if (res.status !== 200) failing.push(`${s.date}/${s.subjectId} -> ${res.status}`);
  }
  if (failing.length > 0) {
    return { name: "swarm sessions resolve", status: "fail", detail: `${failing.length}/${sessions.length} failed: ${failing.join(", ")}` };
  }
  return { name: "swarm sessions resolve", status: "pass", detail: `${sessions.length} sessions all resolved 200 from the API` };
}

// ── Check 3: swarm member roster is the real one, not the demo seed ────
export async function checkMemberRoster(base: string): Promise<CheckResult> {
  const { status, body } = await getJson(`${base}${ROUTES.swarm.members}`);
  if (status !== 200 || !body || typeof body !== "object") {
    return { name: "swarm member roster", status: "fail", detail: `GET ${ROUTES.swarm.members} -> ${status}` };
  }
  const members = (body as { members?: Array<{ id: string }> }).members ?? [];
  // The demo/seed fixture roster is 7 members (issue #344); the real
  // swarm roster is smaller. This check only asserts the seed count is NOT what
  ...
  if (members.length === 7) {
    return { name: "swarm member roster", status: "fail", detail: "7 members returned — matches the demo-seed roster, not the real swarm roster" };
  }
  return { name: "swarm member roster", status: "pass", detail: `${members.length} members: ${members.map((m) => m.id).join(", ")}` };
}
