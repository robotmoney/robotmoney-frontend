...
> **ARCHIVED 2026-09-23 — unadopted infrastructure proposal; not implementation authority or current operating instructions.**
> The [smoke production spec](../technical/smoke-production-spec.md) is the sole adopted deployment design.
> [Release policy](../technical/release-runbooks.md) remains in force.
> The text below preserves the earlier document, including its historical status and recommendations;
> those claims do not authorize work or establish current deployment behavior.
...
Inputs: [`release-runbooks.md`](../technical/release-runbooks.md) (foundational policy),
...
| **Rehearsal fidelity** — prove on a smoke-twin before prod | **Better** | `backup restore --from <prod>` makes the smoke-twin a *whole-stack* smoke-twin, not a database smoke-twin. Repositories are interchangeable across compose and k8s. |
...
**policy §4.4 digital-smoke-twin rehearsal** currently requires restoring "to a local Postgres
...
the smoke-twin a **whole-stack** smoke-twin, not just a database —

```sh
stack deploy   --spec-file <spec> --deployment-dir ~/deployments/smoke-twin
stack manage   --dir ~/deployments/smoke-twin start
stack manage   --dir ~/deployments/smoke-twin backup restore --from <prod-deployment-name>
```
...
| §4.4 smoke-twin | `deploy` + `start` + `backup restore --from <prod>` (§2 above) |
