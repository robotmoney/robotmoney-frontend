...
2. **/allocation** is where the recommendation meets execution. A Vaults section lists the four vaults with Recommended (the latest published robotmoney-allocation recommendation), Applied (the router's effective weights), Actual (each vault's share of the four vaults' TVL) and the two gaps, plus combined TVL, vaults live, tracking error and freshness. Each row links to `/vault/:slug`. The meta row's Contract becomes Router. Asset-level holdings live on the vault pages only: /allocation keeps each sleeve's target constituents (the recipe).
