...
| **F3** | `extras` (spx/eth chart overlay) never written by the pipeline → `NULL` in production. | **NUMERIC-RISK** | Every production run. Only `db/import-regime-eq.ts` (fixture importer) populates it, so smoke/Playwright shows data production lacks. |
