...
**Idempotent submission, one take per epoch.** Take identity is `(session, member)`, unique server-side ([D49](../decisions.md#d49)). A resubmission on an existing key returns the existing record and the participant treats it as success, so a crash after submit or an old/new container overlap during a roster change produces at most a redundant request, never a second take. There is no amendment inside an epoch: a member that changes its mind says so in the next epoch. One take in flight per participant.
