...
**Idempotent submission, one final take.** A retry of the same submission returns the existing record and the participant treats it as success, so a crash after submit or an old/new container overlap during a roster change produces at most a redundant request, never a duplicate take. An intentional amendment is a different thing and is allowed while the window is open: each is its own signed row, and accepting one marks it final and unsets the member's previous one, so exactly one take per member counts at any instant ([D51](../decisions.md#d51)). One take in flight per participant.
