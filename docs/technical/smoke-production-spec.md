...
**Snapshot N.** No release commit carries `backend/schema/` yet (v0.5.0 predates it), so snapshot N is a pinned fixture from a named commit: `backend/tests/fixtures/snapshots/<last filename>/` holds the four `backend/schema/` files of the commit its `fixture.json` names, byte for byte, each with a sha256 pin. Today that commit is the last one whose snapshot ends at `0078_automation_token_holders.sql`, so every migration from 0079 on is proved on a snapshot-built database. The proof bootstraps that fixture, runs the real migrate run with this checkout's migrations and snapshot, and compares the result with a blank bootstrap of the current snapshot by the normalized catalog and by check 3a. The fixture advances only when a release ships whose commit carries `backend/schema/`. The new fixture is that release commit's four files, stored under its last filename, and the old fixture directory is deleted. Three pins move with it in the same change: `FIXTURE_N` in `backend/tests/support/snapshot-fixture.ts`, the fixture's `ALLOWED` entry in `backend/tests/append-only-no-new-deletes.test.ts` (its `snapshot.sql` declares the append-only triggers, and a stale entry fails that file), and the recorded-difference list in `backend/tests/snapshot-advance.test.ts`. A fixture never advances in the same change as a migration, so every migration is proved against the snapshot before it. Differences recorded against a fixture's own defects only shrink. They are emptied when the fixture advances past them.
