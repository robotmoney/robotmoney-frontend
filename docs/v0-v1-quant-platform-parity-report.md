...
| 1.12 | Transport policy: timeouts, cache, hermetic substitution | none (v0 has no timeouts) | `extract/http.ts:16-57`; `extract/fetch-cache.ts:40-42`; `access/hermetic-source.ts:37-49` | all inputs | **PROVEN-DIFFERENT-DORMANT** | Falls back to floor | v1 imposes 8 s/15 s hard fetch timeouts; **a slow-but-healthy upstream that v0 waits for makes v1 fall back to the persisted floor.** HTTP caching defaults off; normal demo supplies a one-hour TTL. `ANALYTICS_SOURCE=hermetic` replaces every series with a seeded random walk — **any parity observed on demo/e2e is vacuous.** |
