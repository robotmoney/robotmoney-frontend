...
| 2026-08-23 | `20260823-review-data-integrity-aum-correctness` `.md` | `releases-0.3.x` / `fix/aum-correctness`, written at `0666dd2`, revised through `46002ec` | AUM and allocation correctness. A backfilled WETH holding was written at ~$60,000 — a BTC price — because `chain/historical-prices.ts` addressed GeckoTerminal OHLCV by **pool** with no `token=`, and `resolvePoolForToken` ranks pools by 24h volume without checking which side the target sits on. Exonerates batching (#739) and shows the defect is non-deterministic, dating to `5788ba6`. **Not a frozen artefact** — unlike every entry above it carries a live remediation plan (T0.1–T5.3) whose checkboxes are ticked as work lands, so the never-edit rule below applies to its *findings*, not to its task state. Phase 1 (T1.1–T1.4, T3.1, T3.2) shipped in `de5cf06`; T0.2 still blocks the v0.3.0 cutover. |
