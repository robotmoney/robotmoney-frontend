...
| # | Guarantee | Why |
|---|---|---|
| **T1** | **No production credential is used or needed.** The smoke-twin's superuser is created by the container from `POSTGRES_USER`, borrowed from nothing — not a repo-root file, not `doadmin`. A rehearsal manufactures its own throwaway bootstrap role (`rm_twin_bootstrap`, `restore-container.ts`) with a fresh random password every run — this is a general property of twin rehearsals, not specific to this one. | This is what lets migrations run against real data with no production secret in play. **Two clauses of this row were corrected 2026-09-18.** It previously said migrations run "*for real*": they run against real data, but until `RM_TWIN_PRODUCTION_PRIVILEGES=1` they run at a privilege level no cutover has — see T2. It also said `doadmin` "applies migrations at no point in this runbook", which stopped being true with the role taxonomy (`0053`): a release carrying `0054`+ needs a login holding `rm_owner` membership to migrate — `DATABASE_URL` (`rm_app`) is refused `SET LOCAL ROLE rm_owner`. **As of `3b7d20a0`, that login is used on production only when the operator passes `--migrate` at §8.2** — a boot without it skips migration entirely rather than running it automatically, and when it does run, the login is `doadmin`, typed at the terminal for that one run, never `MIGRATE_DATABASE_URL` set ambiently. Before `3b7d20a0`, this row's claim that a `0054`+ release "applies migrations through `MIGRATE_DATABASE_URL`" on every cutover was accurate; it no longer is. |
...
> can inspect anything, and **which of the three actually run depends on
> `--migrate` and `--seed`** (see the boxes below and
> `scripts/lib/smoke.README.md`): this release's migrations, plus `migrate()`'s
> own internal `seed()` rewriting the five `swarm.*` schedule rows (§5.4) — both
> gated on `--migrate`, both skipped without it — and the **archive
> initializer** — `prod-bootstrap.ts` run as `docker compose run --rm … api bun
> run scripts/prod-bootstrap.ts --already-migrated` (`scripts/lib/smoke-main.ts`)
> — which adopts and writes archive rows into the live database (§9.1's check
> table), gated on `--seed`. `smoke:archive` bakes in `--seed` but not
> `--migrate`, so **by default only the archive initializer writes** — append
> `--migrate` yourself when this release also ships a schema change. None of it
> has a down migration (§7 header). Do not run this before Gates B, D, E, C and
> A are all green.

> 🔴 **Migration is opt-in, not automatic, as of `3b7d20a0`.** An `--db
> external` boot no longer runs `migrate()` by default: `rm_app` cannot `SET
> LOCAL ROLE rm_owner`, so an unconditional migrate against a database with any
> migration `0054` or later pending died mid-boot rather than serving anything.
> Check the per-release runbook's migration list before you run §8.2:
>
> - **This release ships migrations you need applied now** — append
>   `-- --migrate` to the command below. You will be prompted, at the
>   terminal, for the `doadmin` password. It is never read from `$HOME/.env`,
>   an environment variable, or a file — typed input only, verified against the
>   server before the migration runs, and cleared from the process the moment
>   it finishes. A wrong password, or a non-interactive shell (no TTY —
>   `--migrate` cannot be scripted or piped into), refuses the boot outright
>   rather than falling back to `rm_app` or proceeding un-migrated.
...
```bash
cd <checkout>
# Append `-- --migrate` (in place of the bare `--`) if this release ships
# migrations — see the box above. You will be prompted for the doadmin
