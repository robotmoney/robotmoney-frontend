# Runbook — leaked demo/CI containers on the shared host
...
containers on it (`scripts/stack/naming.ts`): the demo **stack**, the onboarding
...
demo or an operator's shell — never sweep it from CI. `robotmoney.env=ci` means a
CI run; anything of that class older than one job's ceiling (105 min) is a leak.

```bash
docker ps -a --filter label=robotmoney.env=ci \
  --format '{{.Names}}\t{{.CreatedAt}}\t{{.Status}}\t{{.Label "robotmoney.demo.project"}}'
```

## Clearing one known project

Strictly scoped; safe to run while the standing demo is up.

```bash
docker compose -p <project> -f docker-compose.yml -f docker-compose.demo.yml \
  down -v --remove-orphans           # WEB_PORT/POSTGRES_PORT below are interpolation-only
bun run scripts/demo-clean.ts --project <project>
```
...
```bash
bun run demo:reap -- --dry-run                     # ALWAYS first: reads only, mutates nothing
bun run demo:reap -- --env-class ci --dry-run      # CI leftovers only
bun run demo:reap -- --older-than 30m --dry-run    # widen the window, still read-only
bun run demo:reap                                   # apply (default: all classes, older than 6h)
```
...
taken down. Three guards refuse to act (`scripts/lib/demo-reap.ts`; asserted in
`scripts/tests/unit/demo-reap.test.ts`):

- **G1** the project named in `.agents/demo-state.json` is never touched.
...
on this host. They cannot tell the standing demo from a CI orphan.

## Never do this while the stage demo is up
...
backstop of the day ran `demo:clean` **alone**, which removes volumes only. It
...
2. `demo:clean --project` **exits non-zero** on any surviving resource (bare
   `bun run demo:clean` still exits 0 — a running demo is not an error);
3. an always() `demo:reap --env-class ci --older-than 6h` step clears prior runs'
