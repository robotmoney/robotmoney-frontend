...
fixes while preserving the v0.4.1 application, schema, and judge/receipt
invariants:
...
The release contains the following changes relative to `v0.4.1`:
...
git rev-parse HEAD
git tag --points-at HEAD -l 'v0.4.2-rc.*'
bun install --force
...
1. The restored database passes the v0.4.1-baseline and clean-target checks.
...
If the candidate needs a fix, cut the next `v0.4.2-rc.*`, repeat the preflight
and rehearsal gates, and deploy only that new RC. Do not move the final tag
onto an unverified commit.

After clean production postflight, create `v0.4.2` on the exact commit running
