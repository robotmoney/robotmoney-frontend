...
test("signed apply mints a server-side UUID; re-applying with the SAME key refreshes the pending record", async () => {
  ...
  const row = (await sql`SELECT name FROM swarm_members WHERE id = ${memberId}`)[0] as { name: string };
...
test("POST /api/swarm/apply rejects malformed Ed25519 keys and an invalid signature; accepts a validly-signed application", async () => {
...
test("submit: signature verify/reject, window, duplicate", async () => {
  ...
  const receiptPath = routePath(ROUTES.swarm.take, { id: ok.recommendationId });
  const receiptResult = await handleSwarm(new Request(`http://localhost${receiptPath}`), new URL(`http://localhost${receiptPath}`));
  ...
  const tamperedSessionPath = routePath(ROUTES.swarm.session, { date, subject: subj });
  const tamperedSessionResult = await handleSwarm(
  ...
  const tamperedReceiptResult = await handleSwarm(
...
test("full open→brief→submit→aggregate cycle enriches the session (regime_summary, recommendation, synthesis, memo body)", async () => {
  ...
  const rec: any = s.swarmRecommendation;
...
test("bucket aggregation computes the normalized unweighted mean and attributes only stored non-empty bodies", async () => {
  ...
  const recommendation: any = detail?.session.swarmRecommendation;
...
test("restart-safety (issue #208): re-opening the same session is idempotent (one row); a REPLAYED member take is 409 with exactly one recommendation row", async () => {
  ...
  const sessionRows = await sql`SELECT id FROM swarm_sessions WHERE date = ${date} AND subject_id = ${subj}`;
...
// ── Issue #243: paginate /api/swarm/sessions + drop the member-profile N+1 ─

// Walks every page of GET /api/swarm/sessions for the given query,
...
async function walkSessionsPages(
  ...
  for (let page = 0; page < maxPages; page++) {
    ...
    const req = new Request(`http://test/api/swarm/sessions?${qs}`);
...
test("GET /api/swarm/sessions default: light-projected + cursor-paginated (no big fields, no dupes/gaps across pages)", async () => {
  const n = 5;
  ...
  for (const c of created) {
    ...
    // Everything a list consumer (swarm.js / swarm-overview.js) reads is kept.
    expect(s.id).toBe(c.id);
    expect(s.date).toBe(c.date);
    expect(s.subjectId).toBe(c.subjectId);
    expect("swarmRecommendation" in s).toBe(true);
...
test("GET /api/swarm/sessions?full=1 reproduces the pre-#243 unpaginated/unprojected shape; the light default carries both regimeSummary (issue #357) and synthesis (issue #358)", async () => {
  ...
  // cursor is exhausted) carries the same id/state/swarmRecommendation,
  ...
  expect(lightItem.swarmRecommendation).toBeTruthy();
  ...
  // byte-identical to swarmRecommendation.rationale.
  ...
  expect(lightItem.synthesis).not.toBe(lightItem.swarmRecommendation.rationale);
...
test("GET /api/swarm/sessions: malformed cursor and out-of-range limit are 400s (never a 500 or a silent fallback)", async () => {
  const badCursor = Buffer.from("not json", "utf8").toString("base64url");
  const cases = [
    `http://test/api/swarm/sessions?cursor=${badCursor}`,
    "http://test/api/swarm/sessions?limit=0",
    "http://test/api/swarm/sessions?limit=101",
    "http://test/api/swarm/sessions?limit=abc",
  ];
...
test("GET /api/swarm/members/:id/takes (#243B): collapses list+N-detail into one call — newest first, in-progress states included, doesn't collide with the plain member-detail route", async () => {
  ...
  const memberReq = new Request(`http://test${routePath(ROUTES.swarm.member, { id: m.id })}`);
  const memberRes = await handleSwarm(memberReq, new URL(memberReq.url));
  expect(memberRes?.status).toBe(200);
  expect((memberRes!.body as { id: string }).id).toBe(m.id);

  const takesReq = new Request(`http://test${routePath(ROUTES.swarm.memberTakes, { id: m.id })}?limit=10`);
  const takesRes = await handleSwarm(takesReq, new URL(takesReq.url));
...
test("GET /api/swarm/members/:id/takes: an unknown/never-submitted member gets an empty list, not an error", async () => {
  const req = new Request(`http://test${routePath(ROUTES.swarm.memberTakes, { id: rid("ghost") })}`);
  const res = await handleSwarm(req, new URL(req.url));
  expect(res?.status).toBe(200);
...
test("GET /api/swarm/members exposes rosterCap, seatsFilled, and seatsAvailable, updating on activate/deactivate", async () => {
  ...
  expect(initial.rosterCap).toBe(ic.SWARM_ROSTER_CAP);
  expect(initial.seatsFilled).toBe(0);
  expect(initial.seatsAvailable).toBe(ic.SWARM_ROSTER_CAP);
  ...
  expect(afterApply.seatsAvailable).toBe(ic.SWARM_ROSTER_CAP);
  ...
  expect(afterActivate.rosterCap).toBe(ic.SWARM_ROSTER_CAP);
  expect(afterActivate.seatsFilled).toBe(1);
  expect(afterActivate.seatsAvailable).toBe(ic.SWARM_ROSTER_CAP - 1);

  // Deactivate member (status = 'inactive') — frees 1 seat
  await sql`UPDATE swarm_members SET status = 'inactive' WHERE id = ${memberId}`;
...
test("POST /api/swarm/signing-payload and submit reject unknown stances and unknown top-level fields with 400 and clear error string", async () => {
  ...
  const reqUnknownKey = new Request(`http://test${ROUTES.swarm.signingPayload}`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
  ...
  const resUnknownKey = await handleSwarm(reqUnknownKey, new URL(reqUnknownKey.url));
  ...
  const reqUnknownStance = new Request(`http://test${ROUTES.swarm.signingPayload}`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      memberId: m.id, date: "2026-09-25", subjectId: "woon", nonce: "n1",
  ...
  const resUnknownStance = await handleSwarm(reqUnknownStance, new URL(reqUnknownStance.url));
  ...
  const reqSubmitUnknownKey = new Request(`http://test${ROUTES.swarm.submit}`, {
    method: "POST",
    headers: {
  ...
  const resSubmitUnknownKey = await handleSwarm(reqSubmitUnknownKey, new URL(reqSubmitUnknownKey.url));
  ...
  const reqSubmitUnknownStance = new Request(`http://test${ROUTES.swarm.submit}`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${m.token}`,
  ...
  const resSubmitUnknownStance = await handleSwarm(reqSubmitUnknownStance, new URL(reqSubmitUnknownStance.url));
...
test("two sessions for one subject on one day: the dated route returns the LATEST, the id route returns each", async () => {
  ...
  expect((await get(routePath(ROUTES.swarm.sessionById, { id: first.id })))?.status).toBe(200);
  expect((await get(routePath(ROUTES.swarm.sessionById, { id: crypto.randomUUID() })))?.status).toBe(404);
  // A non-uuid segment must 404 rather than reaching Postgres and throwing.
  expect((await get("/api/swarm/sessions/not-a-uuid"))?.status).toBe(404);
...
test("re-registering an existing member rebinds the key and never creates a second row", async () => {
  ...
  const rows = await sql`SELECT id, status, name FROM swarm_members WHERE id = ${id}`;
  ...
  const keys = await sql`SELECT public_key FROM swarm_member_keys WHERE member_id = ${id} AND active`;
...
test("re-registering an active member does not consume roster capacity", async () => {
  ...
  expect(again).toHaveProperty("token");
  const count = await sql`SELECT count(*)::int AS n FROM swarm_members WHERE status = 'active'`;
  expect(count[0].n).toBe(SWARM_ROSTER_CAP);
