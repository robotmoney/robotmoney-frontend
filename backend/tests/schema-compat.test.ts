...
//
// This block is the VERDICT half: checks 3a and 3b called in-process. The
...
// ───────────────────────────────────────────────────────────────────────────
// The §10 W2 gate, runtime half: the REAL api, at N, boots against N + additive
// ───────────────────────────────────────────────────────────────────────────
//
// Criterion 54: "Old code boots after an additive change while genuine drift
// still fails." Nothing here is called in-process and nothing is a hand-written
// state:
//
//   * version N is a real snapshot bootstrap of backend/schema/ by rm_owner;
//   * version M is produced by the REAL migrate run (`runMigrate`, as rm_owner,
//     under the target lock) through its migrationsDir/snapshotDir seams: a
//     temporary migrations directory holding every real file plus one
//     synthesized ADDITIVE migration, and a snapshot directory holding M's
//     snapshot — N's three parts with the migration's DDL appended, its
//     fingerprint regenerated by bootstrapping it into a SEPARATE blank
//     database, its filename list naming the synthesized file. The run applies
//     the migration, records its compat, reconciles grants and publishes M's
//     manifest, exactly as it would for a newer release;
//   * the OLD CODE is this checkout's api entrypoint, which ships N's filename
//     list and knows nothing of the synthesized file. It is spawned as a real
//     process, logged in as rm_app, and must SERVE;
//   * then genuine drift — a column M declares is dropped — and the same
//     entrypoint must REFUSE, naming it.
//
// The red control swaps the declaration to `breaking`: the same run, the same
...
/**
 * M's snapshot directory: N's parts copied, M's DDL appended to the
 * declaration, the fingerprint regenerated from a real bootstrap of that
 * declaration into a separate blank database, and the filename list extended
 * by `synthesized` with the content hash recomputed over it — what a newer
 * release's backend/schema/ would carry.
 */
async function snapshotDirForVersionM(ddl: string, synthesized: string): Promise<string> {
  ...
  try {
    ...
    try {
      await blank.unsafe("CREATE EXTENSION IF NOT EXISTS pgcrypto");
      await blank.unsafe("SET ROLE rm_owner");
      metadata = await regenerateSnapshotMetadata(blank, root);
    } finally {
      await blank.end({ timeout: 5 });
    }
    ...
    const filenames = [...metadata.filenames, synthesized];
    writeFileSync(
      join(root, SNAPSHOT_FILES.metadata),
      `${JSON.stringify({ ...metadata, filenames, contentHash: hashManifest(declaration, filenames) }, null, 2)}\n`,
      "utf8",
    );
    return root;
  } finally {
...
/** Every real migration plus the synthesized one, for the run's migrationsDir. */
function migrationsDirWith(file: string, text: string): string {
  const dir = mkdtempSync(join(tmpdir(), "rm-compat-boot-migrations-"));
  const real = join(import.meta.dir, "..", "migrations");
  for (const name of readdirSync(real)) {
    if (name.endsWith(".sql")) symlinkSync(join(real, name), join(dir, name));
  }
  writeFileSync(join(dir, file), text, "utf8");
  return dir;
}

/** A snapshot-bootstrapped database (version N), then the REAL migrate run to M
 *  with `compat` declared on the synthesized migration. Returns the database. */
async function databaseMigratedToM(compat: "additive" | "breaking", snapshotDir: string): Promise<string> {
  const name = await createSnapshotTemplate(`compat_boot_${compat}`);
  compatBootDatabases.push(name);
  const migrationsDir = migrationsDirWith(
    SYNTHESIZED,
    `-- compat: ${compat}\n-- metadata_version: ${COMPAT_METADATA_VERSION}\n--\n${ADDITIVE_DDL}\n`,
  );
  const owner = postgres(roleDatabaseUrl(name, COMPAT_BOOT_OWNER), { max: 1, onnotice: () => {} });
  try {
...
describe("criterion 54, runtime — the real api at N serves against N + additive, and refuses genuine drift", () => {
  beforeAll(async () => {
    ...
    await expect(loadSnapshot(compatBootSnapshotDir)).rejects.toThrow(SYNTHESIZED);
  }, 120_000);
  ...
  test("old code SERVES after the real run publishes M with an additive migration; a dropped declared column then REFUSES it by check 3", async () => {
    const name = await databaseMigratedToM("additive", compatBootSnapshotDir);
    const app = roleDatabaseUrl(name, COMPAT_BOOT_APP);

    // The old code does not ship the synthesized file: it is a surplus row.
    expect((await loadSnapshot()).filenames).not.toContain(SYNTHESIZED);

    const served = await bootApi(app);
    try {
      if (served.outcome !== "served") {
        throw new Error(`old code refused N + additive (exit ${served.code}):\n${startupLines(served.run).join("\n")}`);
      }
      expect(startupLines(served.run)).toEqual(["startup_preflight: passed"]);
    } finally {
    ...
    if (refused.outcome === "served") {
      await refused.run.stop();
      throw new Error("old code served a database with a dropped declared column");
    }
    expect(refused.code).toBe(1);
    expect(await portIsBound(refused.port)).toBe(false);
    expect(startupLines(refused.run)).toEqual([
  ...
  test("RED CONTROL: the same run with the migration declared `breaking` — the same old code refuses by check 3b", async () => {
    const name = await databaseMigratedToM("breaking", compatBootSnapshotDir);
    const boot = await bootApi(roleDatabaseUrl(name, COMPAT_BOOT_APP));
    if (boot.outcome === "served") {
      await boot.run.stop();
      throw new Error("old code served a database whose surplus migration is declared breaking");
    }
    expect(boot.code).toBe(1);
    expect(await portIsBound(boot.port)).toBe(false);
    const lines = startupLines(boot.run);
    expect(lines).toHaveLength(1);
    expect(lines[0]).toStartWith("startup_preflight: refused check 3: ");
    expect(lines[0]).toContain(SYNTHESIZED);
    expect(lines[0]).toContain("breaking");
  }, 240_000);
});
