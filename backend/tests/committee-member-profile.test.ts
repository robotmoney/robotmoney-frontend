// Issue #325: the committee-apply payload is deliberately minimal
...
// the issue recommends (option B): POST /api/committee/members/:id/profile,
// behind the member's own bearer token, both at the domain layer
// (ic.updateMemberProfile) and over the real HTTP dispatcher (handleCommittee)
...
// Shares the one ephemeral Postgres the other committee test files use
// (tests/preload.ts) — same isolation convention as committee.test.ts /
// committee-waitlist.test.ts / committee-claim.test.ts: start from a clean
...
function postProfile(id: string, token: string | null, body: unknown) {
  const req = new Request(`http://test${routePath(ROUTES.committee.memberProfile, { id })}`, {
    method: "POST",
    headers: {
...
test("POST /api/committee/members/:id/profile over the real route: 401 without a bearer token, 403 for another member's id, 200 for its own", async () => {
  const a = await activeMember();
  const b = await activeMember();
  ...
  const ok = await postProfile(a.id, a.token, {
    ...
    mode: "pull",
    operator: "peaq",
    avatar: { path: "/avatars/committee/a.jpg", source_url: null, credit: "test" },
  });
  expect(ok?.status).toBe(200);
  expect((ok?.body as any).member.tagline).toBe("peaq's first non-human team member.");
  expect((ok?.body as any).member.avatar).toEqual({ path: "/avatars/committee/a.jpg", source_url: null, credit: "test" });

  // GET /api/committee/members/:id now reflects the profile — the exact
  // content-parity gap the issue reports (previously null for every
  // API-created member).
  const fetched = await handleCommittee(
    new Request(`http://test${routePath(ROUTES.committee.member, { id: a.id })}`),
    new URL(`http://test${routePath(ROUTES.committee.member, { id: a.id })}`),
  );
...
test("POST /api/committee/members/:id/profile rejects unknown fields, an empty body, and malformed biases/avatar", async () => {
  const member = await activeMember();
