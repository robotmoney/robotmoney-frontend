...
// activated_at.
//
// CLEANUP IS DELIBERATELY NARROW — DELETE of exactly the two roster ids, never
// `TRUNCATE swarm_members ... CASCADE`. This suite shares one ephemeral
// Postgres with v0-seed-bootstrap.test.ts and prod-bootstrap.test.ts, whose
// archive rows a cascade would destroy (decision #502). Nothing outside the
// live roster is touched, in either direction.
import { afterAll, afterEach, beforeEach, expect, test } from "bun:test";
import { readFileSync } from "node:fs";
import { join } from "node:path";
import { LIVE_ROSTER, LIVE_ROSTER_IDS, readLiveRoster, seedLiveRoster } from "../src/swarm/roster-seed.ts";
