...
test("no credential → 403 on every new admin route, before body parsing", async () => {
  ...
  expect((await handleAdmin(badBody, new URL(badBody.url)))?.status).toBe(403);
  const badSchedule = new Request("http://x/api/admin/schedules/1", {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
    body: "{not json",
  });
  expect((await handleAdmin(badSchedule, new URL(badSchedule.url)))?.status).toBe(403);
});
...
test("wrong token → 403 on new routes", async () => {
  ...
  expect((await call(req("POST", "/api/admin/jobs/1/retry", "nope")))?.status).toBe(403);
  expect((await call(req("PATCH", "/api/admin/schedules/1", "nope")))?.status).toBe(403);
});

// ── GET /api/admin/overview ─────────────────────────────────────────────────

test("overview: queue counts reflect a freshly inserted job", async () => {
  const before = await call(req("GET", "/api/admin/overview", OPERATOR));
  const beforeCount = (before?.body as { queueCounts: Record<string, number> }).queueCounts.pending ?? 0;
  await insertJob({ status: "pending" });
  const after = await call(req("GET", "/api/admin/overview", OPERATOR));
...
test("overview: production-kind alert distinctions (dead, running-too-long, healthy)", async () => {
  ...
  let res = await call(req("GET", "/api/admin/overview", OPERATOR));
  ...
  res = await call(req("GET", "/api/admin/overview", OPERATOR));
...
test("overview: the AC1 sampler kinds are monitored", async () => {
  ...
  const res = await call(req("GET", "/api/admin/overview", OPERATOR));
...
test("gaps: GET /api/admin/gaps returns one report per registered series, auth-gated the same as every other admin route", async () => {
  const res = await call(req("GET", "/api/admin/gaps", OPERATOR));
...
test("jobs: kind/status/scope filters + cursor pagination + limit bounds", async () => {
  ...
  const byKind = await call(req("GET", `/api/admin/jobs?kind=${KIND}`, OPERATOR));
  ...
  const byStatus = await call(req("GET", `/api/admin/jobs?kind=${KIND}&status=dead`, OPERATOR));
  ...
  const byScope = await call(req("GET", `/api/admin/jobs?kind=${KIND}&scopeType=swarm_session&scopeId=${scopeId}`, OPERATOR));
  ...
  const page1 = await call(req("GET", `/api/admin/jobs?kind=${KIND}&limit=1`, OPERATOR));
  const p1 = page1?.body as { jobs: { id: number }[]; nextCursor: string | null };
  expect(p1.jobs.length).toBe(1);
  expect(p1.nextCursor).not.toBeNull();
  const page2 = await call(req("GET", `/api/admin/jobs?kind=${KIND}&limit=1&cursor=${encodeURIComponent(p1.nextCursor!)}`, OPERATOR));
  ...
  // 400s
  expect((await call(req("GET", "/api/admin/jobs?limit=0", OPERATOR)))?.status).toBe(400);
  expect((await call(req("GET", "/api/admin/jobs?limit=201", OPERATOR)))?.status).toBe(400);
  expect((await call(req("GET", "/api/admin/jobs?limit=abc", OPERATOR)))?.status).toBe(400);
  expect((await call(req("GET", "/api/admin/jobs?cursor=not-valid-base64!!", OPERATOR)))?.status).toBe(400);
  expect((await call(req("GET", "/api/admin/jobs?status=bogus", OPERATOR)))?.status).toBe(400);
  expect((await call(req("GET", `/api/admin/jobs?scopeId=${scopeId}`, OPERATOR)))?.status).toBe(400);
  expect((await call(req("GET", "/api/admin/jobs?createdFrom=not-a-date", OPERATOR)))?.status).toBe(400);
});
...
test("runs: kind/status filters + cursor pagination + 400s", async () => {
  ...
  const byKind = await call(req("GET", `/api/admin/runs?kind=${KIND}`, OPERATOR));
  ...
  const byStatus = await call(req("GET", `/api/admin/runs?kind=${KIND}&status=failed`, OPERATOR));
  const statusBody = byStatus?.body as { runs: { status: string }[] };
  expect(statusBody.runs.every((r) => r.status === "failed")).toBe(true);

  const page1 = await call(req("GET", `/api/admin/runs?kind=${KIND}&limit=1`, OPERATOR));
  const p1 = page1?.body as { runs: unknown[]; nextCursor: string | null };
  expect(p1.runs.length).toBe(1);
  expect(p1.nextCursor).not.toBeNull();

  expect((await call(req("GET", "/api/admin/runs?limit=abc", OPERATOR)))?.status).toBe(400);
  expect((await call(req("GET", "/api/admin/runs?cursor=%%%", OPERATOR)))?.status).toBe(400);
  expect((await call(req("GET", "/api/admin/runs?status=bogus", OPERATOR)))?.status).toBe(400);
});
...
test("retry: clones a dead job, unique dedupe key, audit linking both ids, source unchanged", async () => {
  ...
  const res = await call(req("POST", `/api/admin/jobs/${deadId}/retry`, OPERATOR, { reason: "recovering after transient outage" }));
...
test("retry: non-dead statuses → 409, missing job → 404, no queue rows change", async () => {
  ...
  for (const status of ["pending", "running", "succeeded", "failed"]) {
    ...
    const res = await call(req("POST", `/api/admin/jobs/${id}/retry`, OPERATOR, reason));
  ...
  const missing = await call(req("POST", "/api/admin/jobs/99999999/retry", OPERATOR, reason));
  expect(missing?.status).toBe(404);
  const badId = await call(req("POST", "/api/admin/jobs/abc/retry", OPERATOR, reason));
  expect(badId?.status).toBe(400);
});

test("retry: validation — short reason and unknown fields → 400", async () => {
  const deadId = await insertJob({ status: "dead" });
  expect((await call(req("POST", `/api/admin/jobs/${deadId}/retry`, OPERATOR, { reason: "short" })))?.status).toBe(400);
  expect((await call(req("POST", `/api/admin/jobs/${deadId}/retry`, OPERATOR, {})))?.status).toBe(400);
  expect(
    (await call(req("POST", `/api/admin/jobs/${deadId}/retry`, OPERATOR, { reason: "a valid ten char reason", extra: true })))?.status,
  ).toBe(400);
});
...
test("retry: admin cannot retry analytics producer jobs", async () => {
  for (const kind of PRODUCTION_KINDS) {
    ...
    const res = await call(
      req("POST", `/api/admin/jobs/${deadId}/retry`, OPERATOR, { reason: "analytics producer boundary test" }),
    );
    expect(res?.status).toBe(409);
    const after = await sql`SELECT count(*)::int AS n FROM jobs`;
    expect(after[0].n).toBe(before[0].n);
...
test("schedule toggle: admin cannot enable retired consumer analytics schedules", async () => {
  ...
  const res = await call(
    req("PATCH", `/api/admin/schedules/${row.id}`, OPERATOR, { enabled: target, reason: "operational toggle for test coverage" }),
  );
  expect(res?.status).toBe(409);
...
test("schedule toggle: 400/404/409 for unknown fields, missing, protected fields, non-analytics kind", async () => {
  ...
  // unknown field
  expect((await call(req("PATCH", `/api/admin/schedules/${regime.id}`, OPERATOR, { enabled: true, reason, cron: "* * * * *" })))?.status).toBe(400);
  // missing schedule
  expect((await call(req("PATCH", "/api/admin/schedules/999999", OPERATOR, { enabled: true, reason })))?.status).toBe(404);
  // non-boolean enabled
  expect((await call(req("PATCH", `/api/admin/schedules/${regime.id}`, OPERATOR, { enabled: "yes", reason })))?.status).toBe(400);
  // short reason
  expect((await call(req("PATCH", `/api/admin/schedules/${regime.id}`, OPERATOR, { enabled: true, reason: "no" })))?.status).toBe(400);

  // non-analytics kind
  const [vault] = await sql`SELECT id FROM job_schedules WHERE kind = 'vault.sample_share_price' LIMIT 1`;
  expect((await call(req("PATCH", `/api/admin/schedules/${vault.id}`, OPERATOR, { enabled: false, reason })))?.status).toBe(400);
...
test("audit: filters, cursor pagination, and redaction of credential-shaped keys", async () => {
  ...
  const res = await call(req("GET", `/api/admin/audit?actor=${actor}`, OPERATOR));
  expect(res?.status).toBe(200);
  ...
  const noMatch = await call(req("GET", `/api/admin/audit?actor=${actor}&action=nope`, OPERATOR));
  expect((noMatch?.body as { items: unknown[] }).items.length).toBe(0);

  // 400s
  expect((await call(req("GET", "/api/admin/audit?limit=0", OPERATOR)))?.status).toBe(400);
  expect((await call(req("GET", "/api/admin/audit?cursor=***", OPERATOR)))?.status).toBe(400);
  expect((await call(req("GET", "/api/admin/audit?from=not-a-date", OPERATOR)))?.status).toBe(400);
});
...
test("audit: cursor pagination walks distinct pages", async () => {
  ...
  const page1 = await call(req("GET", `/api/admin/audit?actor=${actor}&limit=1`, OPERATOR));
  const p1 = page1?.body as { items: { id: number }[]; nextCursor: string | null };
  expect(p1.items.length).toBe(1);
  expect(p1.nextCursor).not.toBeNull();
  const page2 = await call(req("GET", `/api/admin/audit?actor=${actor}&limit=1&cursor=${encodeURIComponent(p1.nextCursor!)}`, OPERATOR));
...
test("RM_ENV=ephemeral no longer opens the new admin routes without a credential (D52 (1))", async () => {
  expect(process.env.RM_ENV).toBe("ephemeral");
  const res = await call(req("GET", "/api/admin/overview"));
  expect(res?.status).toBe(403);
  const auditRes = await call(req("GET", "/api/admin/audit"));
