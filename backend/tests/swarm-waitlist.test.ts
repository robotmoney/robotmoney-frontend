...
import { deliverSwarmNotification, type SwarmEmailTransport, type SwarmEmailMessage } from "../src/swarm/notifications.ts";
...
test("POST /api/swarm/waitlist — input validation & privacy bounds", async () => {
  ...
  const rows = await sql<{ email: string; email_norm: string; source: string; notified_at: Date | null }[]>`
    SELECT email, email_norm, source, notified_at FROM swarm_waitlist WHERE email_norm = 'waitlist_user@example.com'`;
...
test("notify-on-seat-open — enqueues outbox + worker jobs on member deactivation and stamps notified_at", async () => {
  ...
  // Deactivate member when roster is full -> frees a seat (active drops below SWARM_ROSTER_CAP)
  ...
  const outboxRows = await sql<{ id: string; kind: string; to_email: string }[]>`
    SELECT id, kind, to_email FROM swarm_notification_outbox WHERE kind = 'seat_open' ORDER BY to_email`;
  ...
  const jobRows = await sql<{ kind: string; payload: any }[]>`
    SELECT kind, payload FROM jobs WHERE kind = 'swarm.send_seat_open_notification'`;
  ...
  const queuedWaitlistRows = await sql<{ notified_at: Date | null }[]>`
    SELECT notified_at FROM swarm_waitlist ORDER BY email`;
  ...
  expect(sentMessages[0].text).toContain(`${config.swarmPublicBaseUrl}/swarm/apply`);

  // Verify notified_at is stamped on waitlist rows
  const waitlistRows = await sql<{ email: string; notified_at: Date | null }[]>`
    SELECT email, notified_at FROM swarm_waitlist ORDER BY email`;
