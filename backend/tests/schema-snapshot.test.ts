...
describe("the real snapshot (backend/schema/) — fingerprint, preflight, bootstrap data, grid columns", () => {
  test("the committed fingerprint is exactly the catalog a real bootstrap produces (regenerate: RM_SNAPSHOT_REGENERATE=1)", async () => {
    ...
    //
    // With the variable set it rewrites the file from a real bootstrap — and
    // this test must still never pass BECAUSE it wrote the file. So the
    // regenerated bytes are compared with the bytes that were committed before
    // the write: a stale snapshot fails here, naming the fix (commit the
    // rewritten file and rerun without the variable), and only a snapshot that
    // was already right passes. In CI the variable refuses outright: a
    // verification run is never also the writer.
    if (process.env.RM_SNAPSHOT_REGENERATE === "1") {
      if (process.env.CI) {
        throw new Error(
          "RM_SNAPSHOT_REGENERATE=1 is set under CI — regeneration is a developer action, and a CI run that " +
            "rewrote backend/schema/snapshot.json would verify the file it had just written",
        );
      }
      const metadataPath = join(import.meta.dir, "..", "schema", "snapshot.json");
      const committed = readFileSync(metadataPath, "utf8");
      await withBlankDatabase(async (db) => {
        await db.unsafe("CREATE EXTENSION IF NOT EXISTS pgcrypto");
        await db.unsafe("SET ROLE rm_owner");
      ...
      const regenerated = readFileSync(metadataPath, "utf8");
      expect({
        snapshotJsonWasCurrent: regenerated === committed,
        fix: "backend/schema/snapshot.json was stale and has been rewritten: review it, commit it, rerun without RM_SNAPSHOT_REGENERATE",
      }).toEqual({
        snapshotJsonWasCurrent: true,
        fix: "backend/schema/snapshot.json was stale and has been rewritten: review it, commit it, rerun without RM_SNAPSHOT_REGENERATE",
      });
    }
