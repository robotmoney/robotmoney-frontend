// `bun run twin:capture` — produce the encrypted backup a digital twin restores.
//
// THE OTHER HALF OF `--db twin`. scripts/lib/restore-container.ts consumes four
...
// package.json exposes it as `twin:capture`, the same way `bootstrap` already
// reaches across.
...
async function main(argv: string[]): Promise<number> {
  ...
  const db = postgres(url, { max: 1, idle_timeout: 10, connection: { application_name: "twin-capture" } });
  ...
  // `gpg --symmetric` leaves no passphrase and produces a backup the twin cannot
  // open.
  //
  ...
  try {
    ...
    // otherwise a silent gpg problem produces artifacts the twin will reject at
  ...
  log(`restore it with:  bun smoke -- --db twin${out === DEFAULT_OUT ? "" : ` --backup-dir ${out}`}`);
  return 0;
}
