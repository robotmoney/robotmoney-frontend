...
// scripts/regime/lib/indicators.js, blob sha
// a752928c673c20aef77cde81888183039b57e1ca, fetched/verified 2026-08-02.
// NO logic changes.
...
/**
 * Regime detection indicator universe.
 *
 * Two live panels: macro (8) and onchain (10). The factor panel (7) is
 * fetched as well and feeds /regime_eq; it does not affect /regime's
 * composite. Each indicator is:
 *   - fetched from a free, no-key source
 *   - percentile-ranked over a 3y rolling window
 *   - sign-aligned so +1 = risk-on
 *   - inverse-correlation weighted within its panel
 *
 * Panel scores are averaged 50/50 into a top-line composite, then
 * bucketed by 3y percentile into risk-off / neutral / risk-on.
 *
 ...
 *
 * Note: the "onchain" panel covers crypto signals broadly, not strictly
 * on-chain metrics — it includes a couple of market-price-derived series
 * (BTC/ETH ratio, ETH 50/200 trend) as risk-regime proxies.
 *
 * Each indicator carries three editorial fields (description, derivation,
 * interpretation) surfaced on the /regime/indicators glossary page and
 * also passed through to the dashboard table for inline lookup.
 */
...
const INDICATORS = [
  ...
  //
  // These five equity risk-appetite indicators are pulled and accumulated in
  // data/regime/raw-indicator-history.csv so they have history available if /
  // when we decide to use them. They are intentionally NOT listed in `PANELS`
  // below, so `computeRegime` does not include them in any panel index, the
  // composite, the regime label, the snapshot's weight tables, or the
  // backtest. /regime and /regime/indicators filter on panel === 'macro' or
  // 'onchain' so these stay invisible on the live pages. Pure data pull.
  {
    id: 'SPHB_SPLV',
...
const PANELS = ['macro', 'onchain'];

const ROLLING_WINDOW_DAYS = 365 * 3;
const COMPOSITE_BUCKETS = { risk_off: 0.33, risk_on: 0.67 };

module.exports = { INDICATORS, PANELS, ROLLING_WINDOW_DAYS, COMPOSITE_BUCKETS };
