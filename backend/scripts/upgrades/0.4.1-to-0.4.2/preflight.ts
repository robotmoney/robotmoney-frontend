 ...
 * A v0.4.2 boot therefore fails unless the operator ran the cutover pre-step
...
export async function roleReadinessCheck(db: Db, { record }: Checker): Promise<void> {
  ...
  if (missing.length > 0) {
    ...
    fail(
      "a v0.4.2 boot cannot migrate without the taxonomy: 0045-0053 create tables/roles, and 0054+ run SET LOCAL ROLE rm_owner. Provision it FIRST (runbook §4.1): scripts/ops/provision-db-role-taxonomy.sh against the primary (docs/runbooks/deployment.md §4.3.1).",
    );
  } else {
...
// Run directly: `bun backend/scripts/upgrades/0.4.1-to-0.4.2/preflight.ts`.
