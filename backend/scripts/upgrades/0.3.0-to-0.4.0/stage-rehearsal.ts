...
const code = await runSmokeTwinRehearsal({
  ...
  onReady: async ({ databaseUrl }) => {
    const proc = Bun.spawn(["bun", `scripts/upgrades/0.3.0-to-0.4.0/postflight.ts`, ...(emit ? ["--emit-receipt=P5.rehearsal"] : [])], { cwd: repoRoot, env: { ...process.env, DATABASE_URL: databaseUrl }, stdout: "inherit", stderr: "inherit" });
