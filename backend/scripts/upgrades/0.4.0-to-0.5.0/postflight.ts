// Postflight for the v0.4.0 -> v0.5.0 rollout. All database checks are
...
export async function runChecks(db: Db, { record }: Checker): Promise<void> {
  ...
  record("migrations", missingRelease.length ? "FAIL" : "PASS", missingRelease.length ? `missing: ${missingRelease.join(", ")}` : `all ${RELEASE_MIGRATIONS.length} v0.5.0 migrations recorded`);
  ...
  record("new-tables", absentNewTables.length ? "FAIL" : "PASS", absentNewTables.length ? `absent: ${absentNewTables.join(", ")}` : `all ${NEW_RELEASE_TABLES.length} v0.5.0 tables are present`);
