...
async function run(): Promise<number> {
  ...
  try {
    ...
    try {
      const checker = createChecker("[restore-check-0.4.1] ");
      await runChecks(db, checker);
      return printVerdict(checker.results, { logPrefix: "[restore-check-0.4.1] ", okAll: "DUMP SAFE FOR 0.4.1", okWithWarnings: "DUMP SAFE FOR 0.4.1", blocked: "DUMP BLOCKED" });
...
if (process.argv.includes("--emit-receipt")) {
  const backup = resolveBackupFiles(backupDir);
  emitReceipt({
    step: "P3.gate-c", exit: code, verdict: code === 0 ? "DUMP SAFE FOR 0.4.1" : "DUMP BLOCKED", startedAt,
