// Read-only preflight for the v0.4.0 -> v0.4.1 rollout. This release carries
// four additive migrations (0045-0048); the checks prove production is a
// clean v0.4.0 baseline with none of them applied yet, so the candidate's
...
export async function runChecks(db: Db, { record }: Checker): Promise<void> {
  ...
  record(
    ...
    unexpectedPending.length || orphans.length
      ...
      : `database migration ledger matches the candidate checkout aside from the ${RELEASE_MIGRATIONS.length} v0.4.1 migration(s) not yet applied`,
  ...
  record(
    ...
    alreadyApplied.length ? `v0.4.1 migration(s) already applied: ${alreadyApplied.join(", ")}` : "no v0.4.1 migration recorded yet",
  ...
  record(
    ...
    existingNewTables.length ? `already exist: ${existingNewTables.join(", ")}` : "v0.4.1's new tables are absent before migration",
  );
}
