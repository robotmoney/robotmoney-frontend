...
// GET /api/swarm/apply/:id — docs/architecture.md §11 R2. Public, redacted
...
const APPLY_STATUS_PREFIX = `${ROUTES.swarm.apply}/`;
...
const RE_APPLICATION_STATUS = templateRe(ROUTES.swarm.applicationStatus);
 ...
 * aligned with frontend/public/views/docs/investment-swarm/participation.html and
 * frontend/public/views/docs/investment-swarm/api-reference.html.
 */
export const handleSwarmOnboardingRoutes: SwarmRouteExtension = async (req, url) => {
  ...
  if (req.method === "POST" && url.pathname === ROUTES.swarm.claimChallenge) {
  ...
  if (req.method === "POST" && url.pathname === ROUTES.swarm.claimToken) {
    const body = await readJsonObject(req);
    const memberId = body && requiredString(body, "memberId", 100);
