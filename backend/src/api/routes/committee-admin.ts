// Committee ADMIN REST surface (issue #152): topics, members, session
// scheduling + roster, guarded lifecycle transitions, and audit filtering.
// Thin transport over committee/admin.ts — every route here is PRIVILEGED
// (X-Admin-Token / isPrivileged(), the same fail-closed guard the rest of the
// committee admin dispatcher and /api/admin use) and every owned route checks
...
const PREFIX = "/api/committee/admin/";
...
// Result envelope from committee/admin.ts already carries {ok, status, error,
...
// allowInsecure=true. committee.ts's live mount omits it (defaults to the
// real global config).
export async function handleCommitteeAdmin(
  req: Request,
  url: URL,
