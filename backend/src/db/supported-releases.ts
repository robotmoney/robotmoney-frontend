...
// before the identity row exists").
//
...
//
// WHY v0.5.0 ALONE. D55 (8): it is the release production runs, and an upgrade
// path from anything older is one no database will take. Adding an entry takes
// a new decision. The list below is v0.5.0's own, read from the tag and pinned
// in backend/tests/fixtures/releases/v0.5.0/release.json with a sha256 per
// file; backend/tests/upgrade-from-release.test.ts fails when the two disagree.

/** One release an upgrade may start from. */
export interface SupportedRelease {
  /** The release tag. */
  readonly tag: string;
  /** Every migration filename the release shipped, in filename (apply) order. */
  readonly migrations: readonly string[];
}

/** D55 (8): v0.5.0 alone. Adding a release takes a new decision. */
...
/** How a ledger differs from one release's list. Both empty means equal. */
...
export function describeUnmatchedLedger(
  ...
  return releases
    .map((release) => {
      ...
      return `against ${release.tag}: ${parts.length > 0 ? parts.join("; ") : "equal"}`;
    })
    .join("; ");
}
