...
// persistence path updater processes (the worker's analytics jobs) are allowed
// to use. Typed calls to the authenticated /api/analytics/* boundary, presenting
...
// Resolve the updater's API wiring from the environment at CALL time (tests and
// the demo flip env per case). ANALYTICS_API_URL points the worker at the api
// process; the default matches the single-box API_PORT default.
...
// Fail-loud startup guard (issue #106 AC): an analytics updater configured to
