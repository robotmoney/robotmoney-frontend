...
// (see backend/src/db/seed.ts + scripts/lib/smoke-main.ts); prod and CI seeds never
...
// Upsert the smoke directory. Safe to call repeatedly (see the file header). All
// work runs in a single transaction so an interrupted run leaves the table intact.
export async function seedSmokeProjects(): Promise<void> {
  const now = new Date();
  ...
  console.log(`seeded ${PROJECTS.length} smoke projects (idempotent)`);
}
