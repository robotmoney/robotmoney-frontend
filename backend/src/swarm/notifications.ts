...
// Absolute links into the public site. config.swarmPublicBaseUrl is already
...
const statusPageUrl = (memberId: string) =>
  `${config.swarmPublicBaseUrl}/swarm/apply/${encodeURIComponent(memberId)}`;
const memberProfileUrl = (memberId: string) =>
  `${config.swarmPublicBaseUrl}/swarm/members/${encodeURIComponent(memberId)}`;
...
// API paths: /swarm resolves through the catch-all in
// frontend/public/assets/js/app/routes.js to views/swarm.html (the live
// roster, subjects and sessions), and how-it-works is the one swarm doc
...
const swarmUrl = () => `${config.swarmPublicBaseUrl}/swarm`;
const sessionDocsUrl = () => `${config.swarmPublicBaseUrl}/docs/investment-swarm/how-it-works`;

export function deploymentSwarmEmailTransport(
  env: Record<string, string | undefined> = process.env,
): SwarmEmailTransport {
  const endpoint = env.SWARM_NOTIFICATION_EMAIL_TRANSPORT_URL;
  if (!endpoint) throw new Error("missing required env var: SWARM_NOTIFICATION_EMAIL_TRANSPORT_URL");
  const token = env.SWARM_NOTIFICATION_EMAIL_TRANSPORT_TOKEN;
  return {
    async send(message) {
...
export async function enqueueApplicationReceivedNotification(
  ...
  const from = config.swarmNotificationEmailFrom;
  if (!from) throw new Error("missing required env var: SWARM_NOTIFICATION_EMAIL_FROM");
  ...
  const payload: NotificationPayload = {
    subject: `We have the Robot Money swarm application for ${memberName}`,
    text: [
      `The application for ${memberName} to the Robot Money Investment Swarm is recorded and waiting on an administrator's review.`,
      ...
      "Your Ed25519 private key never left your machine and we never received it. Keep it too: claiming your swarm token after approval means signing a challenge with that exact key, and there is no recovery path that does not start with an administrator rotating you to a new one.",
    ].join("\n\n"),
  };
  ...
  const rows = await tx<{ id: string; send_generation: string }[]>`
    INSERT INTO swarm_notification_outbox (kind, member_id, from_email, to_email, payload)
  ...
  await tx`
    ...
    VALUES (
      ...
      ${`swarm:application-received-notification:${outboxId}:${sendGeneration}`},
      'swarm_member',
      ${memberId},
      'system:apply'
    )
...
export async function enqueueActivationNotification(
  ...
  // endpoints live in /docs/investment-swarm/api-reference, which is where an
  // implementer goes looking for them.
  const payload: NotificationPayload = {
    subject: `${memberName} has a seat on the Robot Money Investment Swarm`,
    text: [
      `${memberName} has a seat on the Robot Money Investment Swarm. The application was approved and the member record is live.`,
      ...
      `The swarm, and the sessions running now: ${swarmUrl()}`,
  ...
  const rows = await tx<{ id: string }[]>`
    ...
    ON CONFLICT (kind, member_id) DO UPDATE SET updated_at = swarm_notification_outbox.updated_at
    RETURNING id`;
  const outboxId = rows[0].id;
  await tx`
    INSERT INTO jobs (kind, payload, dedupe_key, scope_type, scope_id, requested_by)
    VALUES (
      ...
      ${tx.json(jsonValue({ outboxId }))},
      ${`swarm:activation-notification:${outboxId}`},
      'swarm_member',
      ${memberId},
      'system:activation'
    )
...
export async function enqueueSeatOpenNotifications(
  ...
  const from = config.swarmNotificationEmailFrom || process.env.SWARM_NOTIFICATION_EMAIL_FROM;
  if (!from) throw new Error("missing required env var: SWARM_NOTIFICATION_EMAIL_FROM");

  const waitlist = await tx<{ id: string; email: string }[]>`
    SELECT id, email FROM swarm_waitlist WHERE notified_at IS NULL FOR UPDATE`;
  ...
  for (const w of waitlist) {
    const payload = {
      ...
      text: [
        ...
        `Apply now at ${config.swarmPublicBaseUrl}/swarm/apply`,
      ].join("\n\n"),
      waitlistId: w.id,
    };
    const rows = await tx<{ id: string }[]>`
      INSERT INTO swarm_notification_outbox (kind, from_email, to_email, payload)
    ...
    await tx`
      ...
      VALUES (
        ...
        ${`swarm:seat-open-notification:${outboxId}`},
        'swarm_waitlist',
        ${w.id},
        'system:seat_open'
      )
...
export async function deliverSwarmNotification(
  ...
  const row = (await sql<{
    ...
    FROM swarm_notification_outbox WHERE id = ${outboxId}`)[0];
  ...
  try {
    ...
    await sql`
      UPDATE swarm_notification_outbox
