...
export async function seedLiveRoster(): Promise<number> {
  for (const m of LIVE_ROSTER) {
    // Conflict on HANDLE, not id (issue #685). The id supplied here is only
    // ever used by the INSERT branch — a row this deployment has not seated
    // before — and is generated, never a literal. An EXISTING row keeps the id
    // it already has, which is what makes a re-id migration survive the next
    // boot instead of being reverted-and-duplicated. `id` is deliberately
    // absent from the UPDATE SET list for the same reason.
    await sql`
      INSERT INTO swarm_members (id, handle, status, name, tagline, lens, mandate, biases, mode, operator, avatar)
      VALUES (${crypto.randomUUID()}, ${m.handle}, 'active', ${m.name}, ${m.tagline}, ${m.lens}, ${m.mandate},
